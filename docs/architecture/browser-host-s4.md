...
plans unchanged `examples/signal-demo.form`, lowers its exact local fragment
...
node/request/operation, presentation, evidence, and placement identities
issued by Rust. It
...
operations, values, evidence, identity storage, bounds, cancellation, and
terminal truth remain in Rust. All storage and operation capacity is sealed
before activation and checked for stable capacity afterward; this does not
...
identity, cancellation, exhausted evidence storage, and platform failure. It
