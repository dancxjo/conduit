...
The hosted Copy-a-file operation has no authored path or resource token. Its zero-port Form means only “copy a file.” Planning selects one exact std implementation and binds two protected resource roles from provider-issued inputs: a read-existing source and a create-only or replace-existing destination. The Plan contains only opaque handle identities and exact host, boot, capability, resource class, access, byte, and commit bounds.

Execution lowers that Plan through the ordinary numeric kernel boundary. A one-byte command value requests the single admitted `conduit.host/file-copy-step@1` operation. The std provider resolves both handles against its private path registry, reads at most 4 KiB, writes at most that chunk to a same-directory temporary file, and returns the same command value when another step is required. End of input returns no value and the semantic operation completes. The command is therefore finite and kernel-owned without allocating one value per chunk.
...
Every run returns a receipt naming the request, active Play, Plan, source handle, destination handle, structured result, copied byte count where applicable, and kernel evidence count. Raw paths remain provider-private and are absent from the Form, Plan, receipt, and kernel protocol.
