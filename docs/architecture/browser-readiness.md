...
- The reusable in-memory base belongs to `conduit-runtime::bases`, not the composite fixture.
- A fake browser-style adapter manually completes waits and presentations, delays a connection delivery, injects presentation failure and base disconnect, and inspects structured observations.
...
- `hosts/browser/signal-dom-host.mjs` is the thin actual-browser timer/DOM effect adapter. Its single Chromium proof creates two independent host/boot-bound WASM instances and checks exact request/sign identities, stable sealed capacity, item/byte bounds, cancellation, and honest terminal failure.
...
- `fixtures/forms/triple-signal.conduit` is planned unchanged across std and the two simulations. The compared stdout, DOM-state, and onboard-LED-shaped receipts are conformance data, not three-host acceptance.
