...
- `hosts/browser/signal-dom-host.mjs` is the thin actual-browser timer/DOM effect adapter. Its single Chromium proof creates two independent host/boot-bound WASM instances and checks exact request/clue identities, stable sealed capacity, item/byte bounds, cancellation, and honest terminal failure.
