...
| `escape_motion` | delete/dormant | no motion through a current latch in this migration | Contact currently reaches stop plus a persistent exact latch; any future direction-safe withdrawal needs a separately reviewed location-aware reflex contract rather than restoring the historical verb |
...
| local safety latch/clear | allocator-free persistent multi-hazard envelope with monotonic observation/latch generations; current and latched E-stop, wheel-drop, cliff, contact, tilt, impact, charging, liveness, watchdog, clock, and generation failures remain drive-inhibiting; clear is exact Host/Boot/offer/envelope/hazard/generation/authority/deadline bound and refuses active or stale physical truth | integrate exact Pico inputs/watchdog and std reduced-profile observation sources; contact withdrawal/escape policy; physical induced-failure HIL |
