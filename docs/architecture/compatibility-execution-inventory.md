...
| `crates/conduit-composite` (`src/compatibility.rs`) | retained composite comparison fixture | available only through the explicitly named, non-default `compatibility-fixture`; normal composite builds prepare exact child fragments, finite boundary Cords, operation storage, routes, and Signs before executing them through `conduit-kernel` |
...
The following are not compatibility-execution callers. They import only `conduit_runtime::lowering`: production `StdHost`, browser/WASM runtime, `conduit-embedded-build`, the default `conduit-composite` kernel executor, and the Pico firmware build script. The planner's tests also use lowering only. No production Pico target dependency includes `conduit-runtime`; its host-only build dependency has default features disabled.
