...
| Port-aware kernel protocol/storage | archive/pre-reboot-2026-08-04, crates/conduit-core/src/scheduler.rs and crates/conduit-embedded/src/lib.rs | Numeric ports, plan-admitted host operations, fixed storage, and deterministic evidence are sound quarry concepts | Old broad plan schema, driver catalog, policy stack, and separate hosted/embedded APIs | conduit-kernel hosted/fixed storage, public Operation adapter, and scheduler equivalence; exact-route admission; correlated host completion; late-completion rejection; pressure; closure; cancellation; and Thumb vectors |
