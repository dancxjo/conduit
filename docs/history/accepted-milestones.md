...
| Copy a file | unsafe prototype disabled | tests removed from default tree | no admitted host call | no chooser | no | no | no |
...
- exact semantic-contract/profile/port, host-call/resource/authority/link, and
...
- exact local signal-plan lowering into numeric kernel node, directional port,
  cord, route, host-call, resource, and sign tables, with fail-closed
...
- one actual Chromium explicit external-WebSocket chat proof with two separate
  ...
  host-call identities, finite peer/message/queue/history/value/sign
...
cords, direct route ranges, host-call admission, resource references,
...
fan-in, port widths, and host-call concurrency. The exact two-node
...
General graph installation remains open. Hosted host-call
...
Both bind the admitted monotonic-millisecond host call and timer resource,
...
capabilities, exact typed ports, host-call slots, IRQ facts, values,
...
COM1 presentation, and terminated with zero pending host calls. Fresh
...
before and after play, zero pending host calls, and finite cord, sign,
...
resource, and one maximum-in-flight bounded next-key-event host call. The
...
plan, four queue slots, one pending host call, bounded value and sign
