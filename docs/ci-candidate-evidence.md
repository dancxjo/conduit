...
Merged branches are retired by repository machinery rather than GitHub's immediate automatic deletion. The trusted closed-PR controller first retargets every open dependent to `main`, verifies that each immutable child head stayed exact, dispatches reconciliation of that exact head with the new base, rescans for dependents, and only then deletes the merged branch. Any malformed response, failed retarget, changed child head, refused reconciliation dispatch, or remaining dependent retains the branch and fails the controller visibly.
