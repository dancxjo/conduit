...
Merged branches are retired by repository machinery rather than GitHub's immediate automatic deletion. The trusted closed-PR controller first retargets every open dependent to `main`, verifies that each immutable child head stayed exact, requests trusted PR-head reconciliation through the bounded `ci:reconcile` label, rescans for dependents, and only then deletes the merged branch. Any malformed response, failed retarget, changed child head, refused reconciliation request, or remaining dependent retains the branch and fails the controller visibly.
