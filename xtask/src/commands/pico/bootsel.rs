...
pub fn run_bootsel(args: &PicoArgs) -> PicoResult<()> {
    ...
    carrier.send_raw_stream_frame(QUERY, Duration::from_secs(2))?;
    let mut challenge = [0_u8; 1024];
    let challenge = carrier.receive_raw_stream_frame(&mut challenge, Duration::from_secs(2))?;
    ...
    carrier.send_raw_stream_frame(request.as_bytes(), Duration::from_secs(2))?;
    let mut response = [0_u8; 128];
    let response = carrier.receive_raw_stream_frame(&mut response, Duration::from_secs(2))?;
    if response != ACK {
