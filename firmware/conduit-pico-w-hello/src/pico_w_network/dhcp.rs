...
impl DhcpLeaseState {
    pub const fn new() -> Self {
        Self { active: [None; DHCP_POOL_SIZE] }
    }

    pub fn active_count(&self, now_ms: u64) -> usize {
        self.active
            .iter()
            .filter(|lease| lease.map(|entry| now_ms < entry.expires_at_ms).unwrap_or(false))
            .count()
    }
    ...
    pub fn grant(&mut self, request: DhcpRequest, now_ms: u64) -> Option<DhcpGrant> {
        ...
        match request.message_type {
            1 => self
                ...
                .map(|lease_ip| DhcpGrant { message_type: 2, lease_ip }),
            3 => self
                .reserve(request.client, now_ms, DHCP_LEASE_SECONDS)
                .map(|lease_ip| DhcpGrant { message_type: 5, lease_ip }),
            7 => {
    ...
    fn clear_expired(&mut self, now_ms: u64) {
        for slot in &mut self.active {
            if slot.map(|lease| now_ms >= lease.expires_at_ms).unwrap_or(false) {
                *slot = None;
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn reserved_hostnames_do_not_allocate_leases() {
        for name in [b"hello", b"conduit", b"Gateway", b"PETER", b"motherbrain", b"control"] {
            assert!(hostname_is_reserved(name));
        }
        assert!(!hostname_is_reserved(b"client-1"));
    }
}
