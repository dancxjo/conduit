...
use conduit_embedded_build::{
    ...
    GeneratedEmbeddedPlan, generate_embedded_plan,
};
...
fn main() {
    ...
    let full_plan_hash = embedded_plan.full_plan_hash;
    let embedded_plan_source = embedded_plan.render_rust_module();
    fs::write(output.join("embedded_plan.rs"), embedded_plan_source.as_bytes())
    ...
    let firmware_identity = SemanticHash::from_bytes(final_digest.into());
    write_identity_manifest(&target, &profile, &conduit_revision, &full_plan_hash, &firmware_identity);
    let mut generated = String::from(
        "pub const FIRMWARE_IDENTITY: conduit_core::SemanticHash = \
         conduit_core::SemanticHash::from_bytes([",
    );
    for (index, byte) in final_digest.iter().enumerate() {
        if index > 0 {
            generated.push(',');
        }
...
fn generated_embedded_plan(conduit_revision: &str) -> GeneratedEmbeddedPlan {
...
fn write_identity_manifest(
    ...
    profile: &str,
    revision: &str,
    full_plan_hash: &SemanticHash,
    firmware_identity: &SemanticHash,
) {
    let base = env::var("CARGO_TARGET_DIR").unwrap_or_else(|_| "target".to_owned());
    let path = Path::new(&base)
        .join(target)
        .join(profile)
        .join("conduit-pico-w-hello.identity.json");
    let _ = fs::create_dir_all(path.parent().expect("identity manifest path has parent"));
    let mut manifest = String::new();
    let _ = writeln!(manifest, "{{");
    let _ = writeln!(manifest, "  \"conduit_revision\": \"{revision}\",");
    let _ = writeln!(manifest, "  \"target\": \"{target}\",");
    let _ = writeln!(manifest, "  \"profile\": \"{profile}\",");
    let _ = writeln!(manifest, "  \"full_plan_hash\": \"{full_plan_hash}\",");
    let _ = writeln!(manifest, "  \"firmware_identity\": \"{firmware_identity}\",");
    let _ = writeln!(manifest, "}}");
