...
const REQUEST_PREFIX: &str = "CONDUIT_CREATE_HELLO@1:";
const RESPONSE_SCHEMA: &str = "conduit.pete/create-hello-receipt@1";

pub fn run(args: &PicoArgs) -> PicoResult<()> {
    if !args.pete_capstone {
        return Err("pico hello-create requires --pete-capstone".into());
    }
    ...
    if args.dry_run {
        println!(
            "==> pico hello-create (dry-run): bounded START/FULL/mode-35, hello, SAFE/mode-35; no wheel authority"
        );
        return Ok(());
    }
    let port = resolve_capstone_port(args.port.as_deref())?;
    println!(
        "==> pico hello-create: requesting one attended non-motion transaction from {}",
        port.display()
    );
    ...
    if record["schema"] != RESPONSE_SCHEMA
        ...
        || record["observed_oi_mode"] != "full"
        || record["final_oi_mode"] != "safe"
        || record["ready_cue_command_sent"] != true
        || record["motion_authority_granted"] != false
    {
        return Err(format!("bounded Create hello refused or failed: {record}").into());
    }
    println!("==> pico hello-create: Full and hello completed; SAFE cleanup confirmed");
    Ok(())
}
