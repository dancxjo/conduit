...
const RECORD_MAGIC: u32 = 0x434e_4450;
...
pub fn set_phase(phase: PanicPhase) {
    ...
    watchdog.scratch0().write(|value| *value = RECORD_MAGIC);
    watchdog.scratch1().write(|value| *value = phase as u32);
}

pub fn clear() {
    let watchdog = rp_pac::WATCHDOG;
    watchdog.scratch0().write(|value| *value = 0);
    watchdog.scratch1().write(|value| *value = 0);
}
...
pub fn take(watchdog_peripheral: Peri<'static, WATCHDOG>) -> Option<PanicRecord> {
    ...
    let magic = watchdog.get_scratch(0);
    let phase = watchdog.get_scratch(1);
    watchdog.set_scratch(0, 0);
    watchdog.set_scratch(1, 0);
    if !forced_reset || magic != RECORD_MAGIC {
        return None;
    }
    Some(PanicRecord {
        phase: match phase {
...
fn panic(_info: &PanicInfo<'_>) -> ! {
    let watchdog = rp_pac::WATCHDOG;
    if watchdog.scratch0().read() != RECORD_MAGIC {
