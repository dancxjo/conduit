...
async fn serve_conduit_services(class: &mut InertCdc) -> ! {
    ...
    loop {
        ...
        if create_listen::request_matches(request) {
...
async fn qualification_task(
    ...
    let _ = writeln!(
        disposition,
        "{{\"schema\":\"conduit.pete/capstone-disposition@1\",\"translator_oe\":\"low\",\"power_toggle\":\"low\",\"create_uart\":\"isolated_until_attended_play\",\"charging_indicator\":{{\"gpio\":17,\"active_high\":true,\"level\":\"{}\"}},\"status_led\":{{\"gpio\":20,\"level\":\"low\"}},\"i2c\":{{\"controller\":1,\"sda_gpio\":2,\"scl_gpio\":3,\"hz\":100000}},\"watchdog\":{{\"timeout_ms\":2000,\"feed_interval_ms\":250}}}}",
        charging_level,
    );
...
fn main() -> ! {
    ...
    executor.run(|spawner| {
        ...
        spawner.spawn(qualification_task(class, charging_indicator, status_led).unwrap());
    });
}
