...
use conduit_bluetooth::{
    encode_fragment, fragment_count, BleGattProfile, BleReassembler,
    CONDUIT_BLE_SERVICE_UUID, MAXIMUM_BLE_GATT_PACKET_BYTES,
};
use conduit_core::{
    DistributedLeniaWorker, LeniaLineFrameIdentity, LeniaLineFrameView,
    LeniaWorkerAdmission, LENIA_LINE_FRAME_MAX_BYTES,
};
...
#[gatt_server]
struct Server { conduit: ConduitService }
...
pub async fn run(
    spawner: &Spawner,
    sign: &mut UsbCdc,
    pio0: Peri<'static, PIO0>, dma_ch0: Peri<'static, DMA_CH0>,
    dma_ch1: Peri<'static, DMA_CH1>, pin23: Peri<'static, PIN_23>,
    pin24: Peri<'static, PIN_24>, pin25: Peri<'static, PIN_25>,
    pin29: Peri<'static, PIN_29>,
    fw: &'static aligned::Aligned<aligned::A4, [u8]>,
    btfw: &'static aligned::Aligned<aligned::A4, [u8]>,
    nvram: &'static aligned::Aligned<aligned::A4, [u8]>, clm: &'static [u8],
    runtime: &RuntimeTranscriptIdentity, flash_unique_id: [u8; 8],
) -> ! {
    let (bt_driver, _) = crate::radio::init_cyw43_bluetooth(
        spawner, pio0, dma_ch0, dma_ch1, pin23, pin24, pin25, pin29,
        fw, btfw, nvram, clm,
    ).await;
    ...
    let server = Server::new_with_config(GapConfig::Peripheral(PeripheralConfig {
        name: "Conduit Lenia Pico", appearance: &appearance::computer::GENERIC_COMPUTER,
    })).unwrap();
    ...
    let line = async {
        let connection = match advertise(&mut peripheral, &server).await {
            ...
            Err(_) => { let _ = sign.write_marker("CONDUIT_LENIA_ADVERTISE_FAILED").await; halt().await }
        };
        ...
        let _ = sign.write_marker(if result.is_ok() {
            "CONDUIT_LENIA_REGION_COMPLETE"
        } else { "CONDUIT_LENIA_LINE_LOST" }).await;
...
async fn advertise<'v, 's, C: Controller>(
    peripheral: &mut Peripheral<'v, C, DefaultPacketPool>, server: &'s Server<'v>,
    ...
    let scan_len = AdStructure::encode_slice(
        &[AdStructure::CompleteLocalName(b"Conduit Lenia Pico")], &mut scan)?;
    let connection = peripheral.advertise(
        &Default::default(), Advertisement::ConnectableScannableUndirected {
            adv_data: &data[..len], scan_data: &scan[..scan_len],
        },
    ).await?.accept().await?;
...
async fn serve<C: Controller>(
    stack: &Stack<'_, C, DefaultPacketPool>, server: &Server<'_>,
    connection: &GattConnection<'_, '_, DefaultPacketPool>, sign: &mut UsbCdc,
    runtime: &RuntimeTranscriptIdentity, worker: &mut DistributedLeniaWorker,
) -> Result<(), ()> {
    ...
    loop {
        match connection.next().await {
            ...
            GattConnectionEvent::PairingComplete { bond, .. } => {
                if let Some(bond) = bond { let _ = stack.add_bond_information(bond); }
            ...
            GattConnectionEvent::Gatt { event: GattEvent::Write(event) }
                ...
                let length = event.with_data(|_, data| {
                    if data.len() <= packet.len() { packet[..data.len()].copy_from_slice(data); }
                    data.len()
                });
                ...
                if let Some(bytes) = reassembler.admit(&packet[..length]).map_err(|_| ())? {
                    ...
                    let incoming = LeniaLineFrameView::decode(&frame[..bytes.len()]).map_err(|_| ())?;
                    ...
                    match worker.admit(incoming.chunk).map_err(|_| ())? {
                        ...
                        LeniaWorkerAdmission::ResultReady => {
                            ...
                            send_result(worker, server, connection, runtime, session.unwrap()).await?;
                            return Ok(());
                        }
                    }
                }
...
fn validate_work(
    frame: &LeniaLineFrameView<'_>, runtime: &RuntimeTranscriptIdentity,
    session: &mut Option<[u8; 16]>,
) -> Result<(), ()> {
    let id = frame.identity;
    if id.plan_id != lenia_image::PLAN_ID || id.play_id != lenia_image::WORK_PLAY_ID
    ...
    match *session {
        ...
        None => { *session = Some(id.session_id); Ok(()) }
        _ => Ok(()),
    }
}

async fn send_result(
    worker: &DistributedLeniaWorker, server: &Server<'_>,
    connection: &GattConnection<'_, '_, DefaultPacketPool>,
    runtime: &RuntimeTranscriptIdentity, session: [u8; 16],
) -> Result<(), ()> {
    ...
    while offset < identity.total_cells {
        let chunk_len = worker.encode_result_chunk(offset, &mut chunk).map_err(|_| ())?;
        let frame_len = LeniaLineFrameIdentity {
            plan_id: lenia_image::PLAN_ID, play_id: lenia_image::RESULT_PLAY_ID,
            line_id: lenia_image::RESULT_LINE_ID, source_host_id: lenia_image::HOST_ID,
            source_boot_id: runtime.boot_id(), sink_host_id: lenia_image::RESULT_SINK_HOST_ID,
            sink_boot_id: lenia_image::RESULT_SINK_BOOT_ID, session_id: session,
        }.encode(&chunk[..chunk_len], &mut frame).map_err(|_| ())?;
        send_frame(&frame[..frame_len], sequence, server, connection).await?;
        let view = conduit_core::LeniaRegionChunkView::decode(&chunk[..chunk_len]).map_err(|_| ())?;
...
async fn send_frame(
    frame: &[u8], sequence: u8, server: &Server<'_>,
    connection: &GattConnection<'_, '_, DefaultPacketPool>,
) -> Result<(), ()> {
    ...
    for index in 0..count {
        let length = encode_fragment(frame, sequence, index, profile, &mut packet).map_err(|_| ())?;
        let value = Vec::from_slice(&packet[..length]).map_err(|_| ())?;
        server.conduit.notify.notify(connection, &value, false).await.map_err(|_| ())?;
        if index + 1 < count { Timer::after(Duration::from_millis(5)).await; }
    }
    Ok(())
}
...
async fn reboot_after_terminal(sign: &UsbCdc) -> ! {
    ...
    rp_pac::PSM.wdsel().write_value(rp_pac::psm::regs::Wdsel(0x0001_fffc));
    rp_pac::WATCHDOG.ctrl().modify(|value| value.set_trigger(true));
    loop { cortex_m::asm::wfi(); }
}

async fn halt() -> ! { core::future::pending().await }
