...
export async function startApplication(application) {
  ...
  try {
    ...
    let selection = openFormSelection(inventory, await application.storage.readJson('form-selection'));
    await session.restore();
    let saving = Promise.resolve();
    let selected = await application.storage.readJson('foreground-form');
    ...
    const showSelected = () => {
      ...
      root.querySelector('.surface-number').textContent = 'YOUR BODY / YOUR FORM';
    ...
    function render() {
      ...
      activities.replaceChildren(...body.initial_forms.map(form => {
        ...
        button.addEventListener('click', () => {
          ...
          application.storage.writeJson('foreground-form', selected).catch(fail);
          inspection.hidden = true; surface.hidden = false;
          showSelected();
        });
        return button;
      }));
      if (!play) play = openWorkspacePlay({ host, session, source, inputTarget: input, outputRoot: root.querySelector('[data-form-output]'), onState(state) {
        playback = state;
