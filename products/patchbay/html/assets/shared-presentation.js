export function createPatchbaySharedPresentation(presentation, scope = document) {
  ...
  const collectionSlots = (slotPrefix, requiredChunks) => {
    const container = scope.querySelector(`[data-application-collection="${slotPrefix}"]`);
    if (!container) throw new Error(`missing shared ${slotPrefix} collection`);
    const maximumChunks = Number(container.dataset.maximumChunks);
  ...
  return Object.freeze({
    ...
    boundedActionList(slotPrefix, label, entries) {
      const usedChunks = Math.ceil(entries.length / 16), slots = collectionSlots(slotPrefix, usedChunks);
      for (let slotIndex = 0; slotIndex < usedChunks; slotIndex += 1) {
    ...
    boundedArtifacts(slotPrefix, entries, entriesPerSlot = 4) {
      if (entriesPerSlot < 1 || entriesPerSlot > 4) throw new Error(`shared ${slotPrefix} capacity exceeded`);
      const usedChunks = Math.ceil(entries.length / entriesPerSlot), slots = collectionSlots(slotPrefix, usedChunks);
      for (let slotIndex = 0; slotIndex < usedChunks; slotIndex += 1) {
    ...
    boundedEvidence(slotPrefix, label, lines) {
      const usedChunks = Math.ceil(lines.length / 32), slots = collectionSlots(slotPrefix, usedChunks);
      for (let slotIndex = 0; slotIndex < usedChunks; slotIndex += 1) {
        const root = slots[slotIndex], chunk = lines.slice(slotIndex * 32, slotIndex * 32 + 32), slot = `${slotPrefix}-${slotIndex}`;
        present(slot, { actions: [], nodes: [
