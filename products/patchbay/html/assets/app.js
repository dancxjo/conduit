...
async function joinCurrentBody(){
  ...
  try{window.__patchbayMembership=await joinBrowserBody({bodyUrl:url,wasmBytes:admittedRuntimeBytes,expectedBodyId:state.snapshot.body_workbench.body_id,onState:membership=>{document.querySelector("#body-membership-status").textContent=`Browser membership: ${membership}`;if(membership==="admitted"){button.hidden=true;document.querySelector("#disconnect-body").hidden=false;}if(membership.startsWith("refused:")){button.hidden=false;button.disabled=false;document.querySelector("#disconnect-body").hidden=true;}if(membership==="offline"){button.hidden=false;button.disabled=false;document.querySelector("#disconnect-body").hidden=true;document.querySelector("#body-membership-status").textContent="Browser presence disconnected. Durable Body membership was not revoked.";}renderBrowserMembershipTruth();load();}});renderBrowserMembershipTruth();}catch(error){button.disabled=false;document.querySelector("#body-membership-status").textContent=`Body admission refused: ${error.message}`;throw error;}
}
