...
async function joinCurrentBody(){
  ...
  try{window.__patchbayMembership=await joinBrowserBody({bodyUrl:url,wasmBytes:admittedRuntimeBytes,onState:membership=>{document.querySelector("#body-membership-status").textContent=`Browser membership: ${membership}`;if(membership==="admitted"){button.hidden=true;document.querySelector("#disconnect-body").hidden=false;}if(membership==="offline"){button.hidden=false;button.disabled=false;document.querySelector("#disconnect-body").hidden=true;document.querySelector("#body-membership-status").textContent="Browser presence disconnected. Durable Body membership was not revoked.";}load();}});}catch(error){button.disabled=false;document.querySelector("#body-membership-status").textContent=`Body admission refused: ${error.message}`;throw error;}
}
