...
function requireSnapshot(value) {
  ...
  if(value.body_workbench&&(!Array.isArray(value.body_workbench.reviewed_forms)||value.body_workbench.reviewed_forms.length>16||value.body_workbench.reviewed_forms.some(form=>typeof form.label!=="string"||typeof form.source_document_id!=="string"||typeof form.checked_form_id!=="string")))throw new Error("malformed bounded Body Form inventory");
...
function renderBodyWorkbench(workbench){
  ...
  sharedPresentation.boundedArtifacts("body-workbench-forms",current.active_forms.map(form=>{const identity=`form/${form.checked_form_id}`,reviewed=workbench.reviewed_forms.find(candidate=>candidate.checked_form_id===form.checked_form_id),remove=state.snapshot.presentation.actions.find(action=>action.target===identity&&action.intent==="conduit.intent/remove-form@1"),availability=remove&&actionAvailability(remove);return {title:reviewed?.label??form.checked_form_id,details:availability&&!availability.available?[availability.explanation]:[],actions:remove?[{label:remove.label,run:()=>dispatchBodyWorkload(remove),annotate:button=>{button.disabled=!availability.available;if(availability.explanation)button.title=availability.explanation;}}]:[],definitions:[["Source document",form.source_document_id],["Checked Form",form.checked_form_id]],disclosureLabel:"Exact active Form identity"};}));
  const active=new Set(current.active_forms.map(form=>form.checked_form_id)),query=state.reviewedFormQuery.trim().toLocaleLowerCase(),allAvailable=workbench.reviewed_forms.filter(form=>!active.has(form.checked_form_id)),available=allAvailable.filter(form=>!query||`${form.label} ${form.source_document_id}`.toLocaleLowerCase().includes(query));
  document.querySelector("#body-form-results-status").textContent=query?`${available.length} of ${allAvailable.length} reviewed Forms match`:`${allAvailable.length} reviewed Forms available`;
  sharedPresentation.boundedArtifacts("body-workbench-available",available.map(form=>{const identity=`form/${form.checked_form_id}`,add=state.snapshot.presentation.actions.find(action=>action.target===identity&&action.intent==="conduit.intent/add-form@1"),availability=add&&actionAvailability(add);return {title:form.label,details:[availability&&!availability.available?availability.explanation:"Reviewed and ready for this Body workload."],actions:add?[{label:add.label,run:()=>dispatchBodyWorkload(add),annotate:button=>{button.disabled=!availability.available;if(availability.explanation)button.title=availability.explanation;}}]:[],definitions:[["Source document",form.source_document_id],["Checked Form",form.checked_form_id]],disclosureLabel:"Exact reviewed Form identity"};}));
...
async function joinCurrentBody(){
  ...
  const button=document.querySelector("#join-body");button.disabled=true;state.bodyMembershipInterrupted=false;document.querySelector("#body-membership-status").textContent="Requesting explicit Body admission…";
  try{window.__patchbayMembership=await joinBrowserBody({bodyUrl:url,wasmBytes:admittedRuntimeBytes,expectedBodyId:state.snapshot.body_workbench.body_id,onState:membership=>{const status=document.querySelector("#body-membership-status");status.textContent=`Browser membership: ${membership}`;if(membership==="reconnecting"){state.bodyMembershipInterrupted=true;status.textContent="Browser presence lost. Attempting one bounded return for this same Boot.";}if(membership==="returning"){state.bodyMembershipInterrupted=true;status.textContent="Proving continuity for this same browser Host and Boot…";}if(membership==="admitted"){button.hidden=true;document.querySelector("#disconnect-body").hidden=false;if(state.bodyMembershipInterrupted)status.textContent="Browser presence returned for the same current Boot.";}if(membership.startsWith("refused:")){button.hidden=false;button.disabled=membership==="refused:wrong-body";document.querySelector("#disconnect-body").hidden=true;if(membership==="refused:wrong-body")status.textContent="Invitation refused: it belongs to a different Body than the evidence open here.";}if(membership==="offline"){button.hidden=false;button.disabled=false;document.querySelector("#disconnect-body").hidden=true;status.textContent="Browser presence disconnected. Durable Body membership was not revoked.";}renderBrowserMembershipTruth();load();}});renderBrowserMembershipTruth();}catch(error){button.disabled=false;document.querySelector("#body-membership-status").textContent=`Body admission refused: ${error.message}`;throw error;}
}
function renderBrowserMembershipTruth(){const membership=window.__patchbayMembership;if(!membership)return;presentDefinitions("body-membership-facts",[["Host",membership.hostId],["Boot",membership.bootId],["Membership",membership.state()],["Presence",membership.presenceState()]]);const advertisement=membership.advertisement,available=membership.presenceState()==="available";document.querySelector("#body-offers-title").hidden=false;presentDefinitions("body-membership-offers",[["Offer availability",available?"Available from current presence":"Unavailable · retained advertisement only"],["Profile",advertisement.profile],["Offer generation",advertisement.offer_generation],["Capabilities",advertisement.capabilities.map(offer=>offer.capability_id).join(", ")||"none"],["Planner profiles",advertisement.planner_capabilities.map(offer=>offer.profile_id).join(", ")||"none"],["Resource pools",advertisement.resources.map(offer=>offer.pool_id).join(", ")||"none"]]);renderBrowserCapabilityTruth();}
function renderBrowserCapabilityTruth(){const membership=window.__patchbayMembership,inspector=document.querySelector("#body-capability-inspector"),select=document.querySelector("#body-capability-select"),offers=membership?.advertisement.capabilities??[];inspector.hidden=offers.length===0;if(!offers.length){presentDefinitions("body-capability-facts",[]);return;}const selected=offers.some(offer=>offer.capability_id===select.value)?select.value:offers[0].capability_id;if(select.options.length!==offers.length||offers.some((offer,index)=>select.options[index]?.value!==offer.capability_id)){select.replaceChildren(...offers.map(offer=>{const option=document.createElement("option");option.value=offer.capability_id;option.textContent=`${offer.kind_id} · ${offer.capability_id}`;return option;}));}select.value=selected;const offer=offers.find(candidate=>candidate.capability_id===select.value),ports=offer.inputs.concat(offer.outputs).map(port=>`${port.direction} ${port.port_id}: ${port.value_kind} · ${typeof port.temporal==="string"?port.temporal:JSON.stringify(port.temporal)}`).join(", ")||"none";presentDefinitions("body-capability-facts",[["Capability",offer.capability_id],["Semantic kind",`${offer.kind_id} @ ${offer.kind_contract_revision}`],["Implementation",offer.implementation_id],["Artifact",offer.artifact_id],["Execution profile",offer.execution_profile_id],["Typed ports",ports],["Limits",`active=${offer.limits.max_active_instances} queue-items=${offer.limits.max_queue_items} queue-bytes=${offer.limits.max_queue_bytes}`],["Resource needs",offer.resource_requirements.map(requirement=>`${requirement.class_id} × ${requirement.units}`).join(", ")||"none"],["Host-operation needs",offer.host_operations.map(requirement=>`${requirement.contract_id} · in-flight=${requirement.maximum_in_flight} input-bytes=${requirement.maximum_input_bytes} output-bytes=${requirement.maximum_output_bytes}`).join(", ")||"none"],["Authority requirements",offer.authority_requirements.map(requirement=>`${requirement.contract_id} · ${requirement.subject_kind}`).join(", ")||"none"]]);}
function disconnectCurrentBody(){const membership=window.__patchbayMembership;if(!membership)throw new Error("current browser Body membership is unavailable");membership.close();}
...
document.querySelector("#disconnect-body").onclick=()=>disconnectCurrentBody();
document.querySelector("#body-capability-select").onchange=()=>renderBrowserCapabilityTruth();
