...
const FILES: &[(&str, &str)] = &[
    ("two-fronts.native-frame", "native.png"),
    ("two-fronts.native-receipt", "native.json"),
    ("two-fronts.browser-frame", "browser.png"),
    ("two-fronts.browser-receipt", "browser.json"),
];

pub(super) fn write_two_fronts_commit(
    site_root: &Path,
    evidence_root: &Path,
...
pub(super) fn write_two_fronts_current(
    site_root: &Path,
    evidence_root: &Path,
    evidence: &VerifiedEvidence,
) -> Result<(), String> {
    write_exhibit(
        &site_root.join("current/one-form-two-fronts"),
        evidence_root,
        evidence,
        "../../index.html",
    )
...
fn write_exhibit(
    ...
    if destination.exists() {
        std::fs::remove_dir_all(destination)
            .map_err(|error| format!("cannot replace Two Fronts exhibit: {error}"))?;
    }
    std::fs::create_dir_all(destination)
        .map_err(|error| format!("cannot create Two Fronts exhibit: {error}"))?;
