//! Exact cross-face Home evidence correlation without collapsing execution identities.
...
pub(super) fn run(
    ...
    receipts.sort_by_key(|receipt| {
        REQUIRED_FACES
            .iter()
            .position(|face| *face == receipt.face_id)
            .unwrap_or(usize::MAX)
    });
    ...
    std::fs::OpenOptions::new()
        ...
        .map_err(|error| format!("create new Home cross-face index: {error}"))?;
    println!("HOME CROSS-FACE INDEX COMPLETE: {}", output.display());
    Ok(())
}
...
fn verify_artifact(receipt: &HomeFaceReceipt, source: &Path) -> Result<(), String> {
    ...
    let artifact = source
        .parent()
        .ok_or_else(|| format!("{} receipt has no parent", receipt.face_id))?
        .join(relative);
...
fn validate(receipts: &[HomeFaceReceipt]) -> Result<(), String> {
    if receipts.len() != REQUIRED_FACES.len() {
        return Err(format!(
            "Home cross-face index requires exactly {} receipts",
            REQUIRED_FACES.len()
        ));
    }
    ...
    for receipt in receipts {
        if receipt.schema != SCHEMA {
            return Err(format!("{} has the wrong receipt schema", receipt.face_id));
        }
        if !REQUIRED_FACES.contains(&receipt.face_id.as_str()) || !faces.insert(&receipt.face_id) {
            return Err(format!(
                "unknown or duplicate Home face {}",
                receipt.face_id
            ));
        }
        ...
        for (name, value) in [
            ...
            if value.is_empty() || value.len() > 512 {
                return Err(format!("{} has invalid {name}", receipt.face_id));
            }
        }
        ...
        if !host_boots.insert((&receipt.host_id, &receipt.boot_id)) {
            return Err("Home faces collapsed distinct Host/Boot identity".into());
        }
        if !plan_plays.insert((&receipt.plan_id, &receipt.active_play_id)) {
            return Err("Home faces collapsed distinct Plan/Play identity".into());
        }
        if !manifestations.insert((&receipt.renderer_id, &receipt.manifestation_id)) {
            return Err("Home faces collapsed distinct renderer manifestation".into());
        }
        if receipt.face_id == "voice-physical" {
    ...
    {
        return Err("Home cross-face index is missing a required face".into());
    }
    Ok(())
}
...
mod tests {
    ...
    fn receipt(face_id: &str, index: usize) -> HomeFaceReceipt {
        HomeFaceReceipt {
            schema: SCHEMA.into(),
            ...
            artifact_path: format!("face-{index}.json"),
            artifact_sha256: format!("sha256:{:064x}", index + 1),
            voice_physical: (face_id == "voice-physical").then_some(VoicePhysicalAcceptance {
                microphone: true,
                playback: true,
                intelligible: true,
                safe_volume: true,
    ...
    fn complete() -> Vec<HomeFaceReceipt> {
        REQUIRED_FACES
            .iter()
            .enumerate()
            .map(|(index, face)| receipt(face, index))
            .collect()
    }

    #[test]
    fn exact_distinct_faces_form_one_complete_index() {
    ...
    fn artifact_digest_and_containment_are_verified() {
        let root =
            std::env::temp_dir().join(format!("conduit-home-cross-face-{}", std::process::id()));
        std::fs::create_dir_all(&root).unwrap();
        let source = root.join("receipt.json");
        let artifact = root.join("face.json");
