...
function __fish_conduct_global_optspecs
    string join \n check explain run format= diagnostic-format= color= q/quiet v verbose-diagnostics compile-input= compatibility-demo enable-file-write enable-file-watch enable-storage-cache enable-process-exec enable-socket-loopback h/help V/version
end
