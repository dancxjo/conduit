# Fresh host reports and deterministic resolution current form

Status: candidate normative C5 contract. `conformance/c5/host-resolution.json`
...
Capability-report current schema optionally binds one exact realm, entity, passport
...
current schema reports did not bind realm membership or passport status. current schema
...
`membership = none`; policies that require it fail closed on absent current-schema
membership. No current-schema identity is reinterpreted as current schema.
