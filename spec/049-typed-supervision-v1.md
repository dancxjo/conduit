# Typed supervision and terminal handling version 1
...
choice identity. Version 1 has seven actions:
...
1 remains frozen and rejects `supervise` with `CND-SRC-007`; self, missing,
duplicate-subject, or cross-boundary bindings return structured source
diagnostics.

Source-AST schema 3 and lowered-source schema 3 retain each relationship as:
...
or fallbacks. The frozen lowered-source-v2 topology remains nested within the
v3 document. Existing grammar-v1 source and lowered v1/v2 identities do not
change.

## Exact plan version 15
...
implementation/host whose supported plan range excludes version 15 fails
before activation.

Changing any action, limit, target, timer, policy, or allocation changes exact
plan identity. Persisted exact-plan document `conduit.execution-plan/v7`
round-trips all version-15 supervision fields.
...
- SUP-012: source-AST v3, lowering v3, and plan v15 preserve exact identities
  without rewriting frozen prior schemas.
...
The normative fixture is `conformance/c4/supervision-v1.json`. Its 49 named
cases are independently dispatched by the hosted reference test. Schema-15
