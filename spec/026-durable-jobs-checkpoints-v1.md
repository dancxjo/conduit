# Durable finite jobs and checkpoints version 1

Status: C4 normative portable contract

Checkpoint envelope schema version: 1

ExecutionPlan schema version: 6
...
Plan schema 6 adds `jobs` separately from cords, event streams, source, and
...
and evidence allocations. Its progress stream is a separate plan-v5
...
incapable provider fails `CND-JOB-016`. Plans v1 through v5 cannot carry job
facts and retain their frozen identities. Re-lowering a durable job creates a
plan-v6 identity.
...
The normative fixture is `conformance/c4/durable-job-v1.json`.

## Migration

Checkpoint schema v1 and job contract v1 are new. There is no implicit
conversion from a mutable status row, an unversioned state blob, or a
plan-v1–v5 runtime. Hosted tooling must re-lower an authored durable job to
plan v6 with resolved providers and budgets. Existing plan-v1 through plan-v5,
ExecutionEvent v1, and Resonance v1 identities are unchanged.
