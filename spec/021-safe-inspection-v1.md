# Safe multi-artifact inspection version 1
...
`conduit.inspection/v1` report is a presentation-neutral observation about an
...
| Kind | Frozen input boundary | Validation |
|---|---|---|
...
| conformance cases | normative JSON-vector or TSV bytes | frozen suite/header markers and bounded collection structure |

Lowered source and exact plan currently have semantic Rust/core schemas but no
frozen standalone hosted byte codec. The typed adapters inspect them exactly.
...
Human inspection results use stdout. JSON is one `conduit.result/v1` envelope
whose `operation` is `inspect` and whose `result` is one
`conduit.inspection/v1` report. Human or diagnostic-v1 JSON failures use
...
Explicit `--type` is authoritative only when it agrees with a frozen marker.
...
digests. There is no decompression or archive extraction in version 1.
...
| Code | Meaning |
|---|---|
| `CND-INSP-001` | no supported frozen marker |
...
| `CND-INSP-008` | semantic type has no frozen standalone hosted byte encoding |
...
`conformance/c3/inspection-v1.json` freezes 50 positive, negative, boundary,
...
| INSP-003 | Detect only frozen markers and fail closed on unknown, ambiguous, conflicting, or unsupported input |
...
| INSP-011 | Keep human/result stdout distinct from human/diagnostic-v1 stderr |
