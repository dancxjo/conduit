# Structural flow contract version 1

Status: C4 normative portable contract

ExecutionPlan schema version: 4
...
ExecutionPlan v4 records each multi-edge output as one exact `PlanFanOut`:
...
ExecutionPlan v4 pins each merge ID, node, ordered input cord list, ordering
...
V4 semantic identity includes fan-out mode, exact branches, duplicator facts,
...
V1-V3 identities are frozen and remain readable by their matching validators.
Adding structural arrays to an old schema is rejected. A legacy source with
multi-edge topology is migrated by re-lowering the exact source to v4 with an
explicit author/compiler choice; its old plan is not silently reinterpreted.
...
- STR-013: plan-v4 identity covers every structural execution fact.
- STR-014: v1-v3 identities are preserved and migration is explicit.

The normative fixture is
`conformance/c4/structural-flow-v1.json`.
