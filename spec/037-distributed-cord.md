# Distributed cord protocol current form

Status: normative C5 contract. `conformance/c5/distributed-cord.json`
...
ExecutionPlan current schema carries one `PlanDistributedCord` for every cross-host
...
current form offers only `at-most-once` and `at-least-once` (`DST-005`).
...
log. current form does not introduce a new pressure behavior.
...
A cross-host cord requires a current distributed binding with exact peer,
authority, backend, protection, carrier endpoint, resource, and retry facts.
A local cord is never reinterpreted as distributed from ambient host state.
Specification 040 defines the corresponding transport artifact and execution
profile pins.
