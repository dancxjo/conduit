# Independent inhibit plane and hazardous-host profile v1
...
3. `PlanHazardClosure` schema 14 binds the exact profile and observation;
...
**INH-003.** Execution-plan schema 14 extends the schema-13 hazard closure with
a bounded set of exact hazardous-host bindings. Schema 13 identities remain
unchanged when this set is absent. A schema-14 hazardous closure requires at
...
`conformance/c4/inhibit-plane-v1.json` contains 29 independently dispatched
cases. The core reconstructs and executes every case. Compiler tests seal and
round-trip a schema-14 hazardous-host binding and prove a later run-start
