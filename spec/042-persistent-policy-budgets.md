# Persistent policy budgets current
...
Effect requirements may name an optional `policy_budget_class`. Absence means
that no policy budget is required. Presence requires one or more
`PlanPolicyBudget` bindings whose exact resource class matches and whose action
equals the effect action.

The current execution-plan schema retains each policy, status, optional lease, required
...
`conformance/c2/persistent-budget.json` contains 15 independently dispatched
