# Immutable ExecutionEvent evidence and provenance current form

Status: normative current contract

ExecutionEvent schema marker: `0`

NDJSON representation schema marker: `0`
...
propagation of these correlation fields without changing event current schema.
...
`conduit/execution-event` current schema. The identity field itself is excluded.
...
It never sorts by timestamp. Replaying the current stream yields the same
...
`conformance/c2/execution-event.ndjson` is the current streaming fixture. It
...
`conformance/c2/execution-event.tsv` freezes causation, nested paths,
...
| EVD-010 | Round-trip current NDJSON and in-memory records without semantic drift |
