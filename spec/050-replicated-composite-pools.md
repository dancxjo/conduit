# Bounded replicated composite pools current form
...
The runtime consumes the current `.panel` pool grammar and lowering identity
...
Every current replicated-pool entry has one `PlanPoolRuntime`. An entry without
that runtime contract is not an executable current plan.
...
from current-schema plan entries, rejects a host profile whose arrays are too
...
- `CND-POOL-HOST-002` current pre-current-schema pool is not executable
...
| POL-015 | Consume current source/lowering semantics and reject displaced runtime execution rather than reinterpret them |
