# Administrative containment current
...
and preserves the current effect identity. Presence requires an
...
Execution-current plan schema adds optional `administrative_subject` and
...
Hosted administrative plans use `conduit.execution-plan`; ordinary hosted
plans remain `conduit.execution-plan`/current plan schema. Compile-input current adds
...
`conformance/c2/containment.json` contains 33 independently dispatched
