# Resonance typed event streams current form

Status: C4 normative portable contract

Resonance contract schema marker: `0`
...
- normative executor evidence current by specification 012;
...
separate current plan `PlanEventStream`. Logs, UI messages, and projection rows are
not normative evidence.

## Compatible shared envelope

`ResonanceEnvelope` extends rather than replaces `ExecutionEvent` current. The
...
current event identity remains valid and unchanged.
...
Plan current pins publisher, complete `EventStreamContract`, resolved provider
...
Current plan identity includes every stream fact. Repository-owned plans are
regenerated from current lowered source.
...
- RSN-002: extend current ExecutionEvent current compatibly.
...
- RSN-014: preserve current plan identities and add exact current plan stream identity.
...
The normative fixture is `conformance/c4/resonance.json`.
