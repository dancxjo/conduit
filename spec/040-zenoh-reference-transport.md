# Zenoh reference transport current
...
2. the current plan distributed binding;
...
The distributed binding is part of the current plan representation.
Plan requires distributed-binding current schema. current schema adds:
...
host observations, and allocation remain pinned. A current-schema artifact MUST be
present exactly in the plan artifact set. Missing or mixed revisions reject;
current schema is not reinterpreted.
...
`conduit/embedded-host-service` adapter and
`conduit/ffi-message` ABI. Fixed request/reply values cross the existing
...
| ZEN-002 | Plan pins the complete current-schema transport selection without reinterpreting current plan. |
