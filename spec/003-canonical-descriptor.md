# Canonical descriptor form current form

Status: normative current contract

Canonical form schema marker: `0`
...
A current form descriptor is the following concatenation:
...
current form has no floating-point value. A schema may define a decimal, unit
...
- rejects lengths that cannot fit in the current form unsigned 64-bit length.

The depth limit is part of current form portable acceptance behavior. Hosted
...
identify current form deliberately.
...
- lengths outside the current form range;
...
## Current vectors and independent reader

`conformance/c1/canonical.json` is normative. It covers:

- reordering map fields;
- reordering set members;
- omitted schema defaults;
- excluded annotations;
- every current form scalar category;
...
current form is immutable. A future canonical form:

- uses a new magic version;
- uses a new semantic-hash domain separator;
- publishes new current vectors and at least one independent reader;
- does not reinterpret current form bytes; and
...
| ID | Obligation |
|---|---|
| CAN-001 | Emit the exact current form descriptor envelope and value tags |
...
| CAN-009 | Preserve current form forever and migrate through decoded semantics |
