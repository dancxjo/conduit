# Exact compilation and heterogeneous packages current form

Status: normative C5 contract. Requirement identifiers `CMP-001`
through `CMP-013` and `PKG-001` through `PKG-018` are exercised by
`conformance/c5/compile-package.json` and
`conformance/c5/compile-source-limits.json`.
...
document. current compile-input schema pins:
...
The result uses the finite `conduit.result` stdout envelope. Human status
...
primitive, round-trips through the current-schema exact-plan decoder, and passes
portable validation without provider access or execution (`CMP-009`).

current compile-input schema retains the current-schema planning model, and its
`conduit.execution-plan` document predates the live distributed-session
requirements in specification 037. This workflow
therefore rejects a placement whose cord endpoints select different hosts
instead of emitting an older plan with hidden transport semantics. A current-schema
...
The current compile-input schema requires all four explicit source limits and
the marker `0`; ambient source limits are never consulted. Producers reseal
the compile-input identity whenever a limit changes. This does not change the
canonical primary `conduct` invocation or execution-plan document schema.

The additive `conduit.execution-plan` document carries current plan typed
supervision bindings from specification 049. Compile input must supply one
exact action/resource/timer binding for every lowered current grammar supervision
relationship. Missing, extra, incompatible, or underallocated bindings fail
before a partial plan is emitted.

The additive `conduit.execution-plan` document carries current plan executable
pool runtime bindings from specification 050. Each current lowered pool must
...
The current binary envelope is deliberately pathless:
