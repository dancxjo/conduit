...
`conduit.run/v1` remains frozen in specification 020 and
`conformance/c3/conduct-output-v1.json` as the exact pre-release contract that
...
`conduit.run/v2`. Readers recognize v1 only to reject it as withdrawn and
reject every other unsupported schema/version combination without fallback.

Finite `conduit.result/v1`, diagnostics, human output, and the ordinary
...
Legacy runtime stdout and stderr writes become `channel_chunk` records:

```json
{"schema":"conduit.run/v2","schema_version":2,"sequence":7,"record":"channel_chunk","channel":"stdout","encoding":"hex","payload_bytes":3,"payload_hex":"616263"}
```
...
The v2 implementation provides a direct `execution_event` path accepting the
...
the v2 outer schema. It is a transport completion fact, not immutable
...
`conformance/c3/conduct-run-stream-v2.json` freezes limits, split and
...
The historical v1 fixture is unchanged. This is intentional evidence of the
withdrawn contract, not a second supported writer. Migrating a consumer means
selecting v2 and treating `channel_chunk` only as compatibility I/O. A
...
| ID | Obligation |
|---|---|
| RUN-001 | Emit current run NDJSON exclusively as tagged `conduit.run/v2` records |
| RUN-002 | Preserve v1 bytes as historical fixtures while explicitly rejecting the withdrawn writer |
