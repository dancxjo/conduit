# Conduit conformance harness current form
...
`conformance/current/manifest.json` is the current form inventory. It names:
...
```json
{
  ...
  "request_id": "flow/flow-policy/block",
  "suite": "flow",
  "fixture": "flow-policy#block",
  "profile": "conduit.c2",
  "operation": "bounded-flow-transition",
  "requirement_ids": ["FLW-001", "FLW-002"],
  "result_fields": ["disposition", "events", "outcome", "queue_after", "reason", "state", "transition"],
  "environment": {
    "clock": {"basis": "conduit.fixture-clock", "tick": 1000},
    "seed": 1380991557,
    "host_observations": []
  },
  "input": {}
...
With no manifest argument, the current form manifest is selected.
...
- A **fixture correction** retains `fixture_version`, increments
  ...
  correction entry to `conformance/current/CHANGELOG.md`. A correction may repair
...
current form artifacts are retained. A new version does not rewrite the meaning of
an old request.
