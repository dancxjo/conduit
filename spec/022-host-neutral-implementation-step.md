# Host-neutral implementation and bounded step contract current form
...
- `ExecutionProfile` identifies exact execution limits and representation
  bindings;
- `ExecutionPlan` current schema pins that profile to one primitive instance;
...
`conduit/implementation-execution-profile` current schema contains:
...
The current plan requires exactly one valid profile on every primitive node and
adds a domain-separated node/profile fact to plan identity. A profile mutation
without a matching canonical hash is rejected. Omitting the profile is invalid.
...
- an explicit current-form request/reply adapter suitable for a process or WASM
  component transport.
...
`conformance/c4/implementation-step.json` freezes 45 positive, negative,
boundary, migration, native/message-equivalence, and malicious-profile cases.
The allocator-free reference tests cover exact profile identity, plan-current-schema
...
| IMP-003 | Pin one exact canonical execution profile per primitive in the current plan |
