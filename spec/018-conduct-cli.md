# Conduct command-line contract current form
...
finished line. current form may emit `Checking`, `Resolving`, `Running`, and
...
No current operation exposes a truthful, bounded progress total, so current form
...
`conformance/c3/conduct-cli.json` is normative. Command cases execute the
...
| CLI-007 | Apply the current terminal, environment, and explicit color precedence |
