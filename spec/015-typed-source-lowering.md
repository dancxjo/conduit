# Conduit typed source lowering current form
...
current form has these explicit stages:
...
Specifications
[`017-port-groups-correlation.md`](017-port-groups-correlation.md) and
[`019-source-lowering.md`](019-source-lowering.md) define the current lowering
closure: exact keyed-member spans, complete contract direction validation,
explicit plan maxima, correlation identity, ordinary cords, child
relationships, exports, bindings, unresolved constraints, and selected-root
input. These facts share the single schema marker `0`.
...
`conformance/c3/source-lowering.json` is normative, language-neutral input.
It declares the exact fixture catalog and literal grammar and covers:

- every current form literal form;
...
| LWR-002 | Parse and retain the exact closed current form literal forms without binary-float identity |
