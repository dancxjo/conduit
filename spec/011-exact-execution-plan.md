# Exact ExecutionPlan identity and portable validation current form

Status: normative current contract

ExecutionPlan schema marker: `0`

This document defines the one current pre-release plan form. The topical
specifications for port groups, execution profiles, structural flow, event
streams, durable jobs, satisfaction proofs, runtime evidence, distributed
cords, containment, persistent policy, hazard closure, inhibit observations,
typed supervision, and replicated pools define fields in that same current
form. They do not create a reader ladder.

An execution-plan marker other than `0` is rejected. Repository-owned plans are
regenerated from current lowered source, sealed observations, and exact
resolution inputs. A displaced plan is neither reinterpreted nor upgraded by a
...
`PlanGraph` is the semantic topology used for node/cord compatibility
...
A runnable current plan has an empty unresolved-selector collection. The schema can
...
is structurally input. The current schema admits a non-exact pair only through the complete compatible
...
The current schema pins the resolved admission, supervision, fallback,
restart, deadline, idle, cleanup, per-instance, queued-request, evidence, and
old/candidate/rollback generation contracts from specification 050. Its
aggregate reservation covers generation
...
`conduit/execution-plan` current schema. The identity field itself is excluded.
...
instance pool/grant relation, current plan supervision/action/target relation, and
current plan pool-runtime/reservation/generation relation, and unresolved
selector.
...
6. one exact bounded distributed binding for every cross-host cord and none
   for local cords in current schema;
...
10. current plan supervision subjects, handlers, compatible targets, nesting,
    timers, limits, and complete minimum allocation;
11. current plan pool admission, lifecycle, queue, exact reservation, and
...
`conformance/c2/execution-plan.tsv` freezes valid minimal and nested plans,
