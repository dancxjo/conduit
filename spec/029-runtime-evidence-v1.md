...
feedback state. Version 1 therefore records an executor-local opaque handle
...
Plan schema 8 adds at most one `RuntimeEvidencePolicy`. The policy is part of
...
V1–v7 plans remain readable with frozen identities and no runtime-evidence
policy. A v8 plan can explicitly disable instrumentation with no stream and
all bounds zero. Recording requires a plan-v5 `EventStreamContract` whose
...
`conduit/runtime-observation` version-1 payload:
...
Specification 028 provides the direct bounded `conduit.run/v2`
...
| `CND-RTE-011` | projected event fails the frozen ExecutionEvent contract |
| `CND-RTE-012` | bounded hosted allocation, handle index, or record-size accounting failed |

## Conformance and migration

`conformance/c4/runtime-evidence-v1.json` freezes explicit disablement,
plan-v8 identity, shared Resonance publication, lifecycle, pressure/loss,
...
Migration from v1–v7 re-lowers the same exact source/constraints into v8 with
an explicit disabled or recording policy. Existing plan identities are never
rewritten.
...
| ID | Obligation |
|---|---|
| RTE-001 | Project only executor-owned observations into frozen ExecutionEvent v1 |
| RTE-002 | Make recording mode, stream, capacity, reserve, and sampling exact plan-v8 identity |
...
| RTE-013 | Preserve v1–v7 plan identities and reject runtime policy fields before v8 |
| RTE-014 | Serialize runtime evidence through the direct bounded run-v2 structured path |
