# Port and typed configuration contracts version 1

Status: stable

Port descriptor schema version: 1

Configuration-field descriptor schema version: 1
...
This specification freezes their version 1 fields, encodings, directional
checks, diagnostics, defaults, mutation timing, identity, and redaction rules.

It builds on exact type references from
[`005-type-contracts-v1.md`](005-type-contracts-v1.md) and the directional
algebra from
...
The descriptor kind is `conduit/port-contract`, schema version 1. All fields
...
`conformance/c2/port-contract-v1.tsv` freezes the complete input descriptor
hash and round-trips each external spelling.

The older C1 `node-contract` canonical vector remains immutable evidence for
canonical-form version 1. It is not silently reinterpreted as this complete C2
...
The field descriptor kind is `conduit/config-field-contract`, schema version 1.
...
Version 1 forbids inline defaults for restricted or secret fields. It also
...
- `conformance/c2/port-contract-v1.tsv` covers direction, type, presence,
...
- `conformance/c2/config-v1.tsv` covers canonical default application,
...
| ID | Obligation |
|---|---|
| PRT-001 | Encode every version 1 port field in canonical semantic identity |
