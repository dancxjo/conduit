...
pub struct PhaseSynchronizationBack {
...
impl Default for PhaseSynchronizationBack {
    fn default() -> Self {
        Self::new()
    }
}

impl<const PORTS: usize> StepBack<PORTS> for PhaseSynchronizationBack {
    fn step(
        &mut self,
        io: &mut StepIo<PORTS>,
...
mod tests {
    ...
    fn pair(
        operation: &mut PhaseSynchronizationBack,
        state: RhythmState,
    ...
    fn stale_outside_and_adjusted_outcomes_remain_distinct() {
        let mut operation = PhaseSynchronizationBack::new();
        assert_eq!(
            pair(
                &mut operation,
                local(4),
        ...
        let mut outside = PhaseSynchronizationBack::new();
        assert_eq!(
            pair(
                &mut outside,
                local(8),
    ...
    fn malformed_missing_duplicate_and_cancel_are_machine_distinct() {
        let mut malformed = PhaseSynchronizationBack::new();
        ...
        let mut missing = PhaseSynchronizationBack::new();
        ...
        let mut no_local = PhaseSynchronizationBack::new();
        let mut close_local = StepIo::test_frame([None; 2], [true, false], [None; 2], None, 4);
        no_local.step(
            &mut close_local,
            &StepInputBytes::test_frame([None; 2], None),
        ...
        let mut cancelled = PhaseSynchronizationBack::new();
