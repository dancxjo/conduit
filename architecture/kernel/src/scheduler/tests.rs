...
use crate::{
    ...
    HostCallOutcome, KernelEventKind, NodeId, Operation, OperationAction, OperationInput, PortId,
    ProtocolError, RemoteEndpointId, RequestId, RouteRange, RouteTarget, SignQuery, SignSink,
...
impl Operation for ConformanceOperation {
    fn start(&mut self) -> OperationAction {
        match self {
            Self::Tick { request, input, .. } => OperationAction::RequestHostCall {
    ...
    E: SignSink + SignQuery,
{
    ...
    routes.seal().unwrap();
    let mut bindings = FixedHostCallBindings::<6>::new(1);
    bindings
        .install(
            NodeId(0),
            HostCallBinding {
        ...
        .unwrap();
    bindings.seal().unwrap();
    let mut scheduler = FixedScheduler::<_, _, _, 6, 5, 2, 5, 12, 5, 6, 2>::new_with_host_calls(
        [
            node([None, None]),
            node([Some(CordId(0)), None]),
            node([Some(CordId(1)), None]),
    ...
    for _ in 0..512 {
        if let Some(request) = scheduler.next_host_request() {
            ...
            assert_eq!(request.operation, HostCallId(0));
