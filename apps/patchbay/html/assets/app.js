...
function renderTimeline(subject){
  ...
  gap.textContent=timeline.gap?`Replay is incomplete: ${timeline.gap.dropped_records} observations lost before sequence ${timeline.gap.first_retained_sequence}. Exact reconstruction across this gap is unavailable.`:timeline.evicted_events?`${timeline.evicted_events} oldest events evicted; available range begins at sequence ${timeline.events[0]?.sequence}.`:"";
  timeline.events.forEach((event,index)=>{if(timeline.subject_filter&&event.subject!==timeline.subject_filter&&event.related_subject!==timeline.subject_filter)return;const row=document.createElement("li"),selectEvent=document.createElement("button"),context=document.createElement("div");row.setAttribute("aria-current",String(projection.cursor===index));selectEvent.type="button";selectEvent.textContent=`seq ${event.sequence} · ${event.event} · ${event.subject} · ${watchValue(event)}`;selectEvent.onclick=()=>dispatchTimeline("select-event",{index}).then(()=>select(event.subject));context.className="timeline-event-context";context.textContent=`Host ${event.host} · Form ${event.form} · Body ${event.execution.body.join("")} · Plan ${event.execution.plan.join("")} · Play ${event.execution.play.join("")}`;row.append(selectEvent,context);list.append(row);});
}
...
async function dispatchTimeline(action,extra={}){
  const timeline=state.snapshot.timeline;if(!timeline)throw new Error("debugger timeline is unavailable");const response=await fetch("/api/debugger-timeline",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({presentation_id:state.snapshot.presentation.identity,presentation_revision:state.snapshot.presentation.revision,timeline_revision:timeline.revision,action,...extra})});if(!response.ok)throw new Error(`debugger timeline HTTP ${response.status}`);const next=requireSnapshot(await response.json());render(next);return next;
}
