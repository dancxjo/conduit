...
import { clearBodyWorkbench, installBodyWorkbench, renderBodyWorkbench, requireWorkbench } from "/assets/workbench-shell.js";
...
function render(snapshot){
  ...
  const list=document.querySelector("#subjects"),navigationSubjects=state.projected.subjects;list.replaceChildren();for(const subject of navigationSubjects){const li=document.createElement("li"),button=document.createElement("button");button.type="button";button.dataset.subject=subject.identity;button.dataset.role=subject.role;button.setAttribute("aria-pressed","false");button.textContent=`${subject.role}: ${subject.accessibility_name}`;button.onclick=()=>select(subject.identity);li.append(button);list.append(li);}renderSeedPalette();renderGearPalette();renderParts();renderBodyWorkbench(snapshot);renderFlow(snapshot,{onSelect:select,onConnect:(source,sink)=>authoringEdit("connect-ports",semanticIdentity(source),{secondary:semanticIdentity(sink)}),onClear:()=>snapshot.navigation?dispatchNavigation({kind:"focus",subject:snapshot.navigation.navigation.places.find(place=>place.place===cursor.place).root_subject}):dispatchInteraction({kind:"clear"}),lens:state.lens});renderStructuredNavigator();
  ...
  renderTemporalContext();const diagnosticLines=subjects("Diagnostic").flatMap(subject=>texts(subject.identity));
...
async function load(){try{const response=await fetch("/api/snapshot",{cache:"no-store"});if(!response.ok)throw new Error(`HTTP ${response.status}`);const snapshot=requireSnapshot(await response.json());if(state.snapshot){const staleSnapshot=snapshot.revision<state.snapshot.revision||(snapshot.revision===state.snapshot.revision&&snapshot.interaction.revision<state.snapshot.interaction.revision),staleEvidence=snapshot.workbench&&state.snapshot.workbench&&snapshot.workbench.evidence_revision<state.snapshot.workbench.evidence_revision;if(staleSnapshot||staleEvidence)throw new Error("stale Body or Presentation revision");if(snapshot.revision===state.snapshot.revision&&snapshot.interaction.revision===state.snapshot.interaction.revision)return;}render(snapshot);}catch(error){clearBodyWorkbench(`Body evidence unavailable: ${error.message}`);document.querySelector("#status").textContent=`Snapshot unavailable: ${error.message}`;}}
...
document.body.dataset.lens=state.lens;document.body.dataset.workbenchView="program";installBodyWorkbench({navigate:dispatchNavigation,invoke:dispatchSemanticAction});load().then(()=>joinCurrentBody()).catch(error=>{clearBodyWorkbench(`Body attachment unavailable: ${error.message}`);document.querySelector("#status").textContent=`Browser Host admission unavailable: ${error.message}`;});window.addEventListener("online",load);window.setInterval(load,250);window.patchbayReload=load;
