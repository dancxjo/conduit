...
function renderWatches(subject){
  ...
  const summaries=[],history=[];for(const watch of set.watches){
    ...
    for(const projection of watch.learned_projections??[])learned.append(renderLearnedProjection(projection));
  }sharedPresentation.boundedArtifacts("watch-list",summaries);sharedPresentation.boundedArtifacts("watch-history",history);
}
...
function renderSeedPalette(){
  ...
  sharedPresentation.boundedList("seed-results","Available Seeds",visible.map(subject=>({identity:subject.identity,text:subject.label,detail:texts(subject.identity)[0]??subject.accessibility_name,run:()=>openSeed(subject.identity),annotate:button=>{button.dataset.seed=subject.identity;button.dataset.subject=subject.identity;button.setAttribute("aria-label",`Open Seed ${subject.label}`);}})),null);
  presentSharedStatus("seed-results-status",`${visible.length} of ${all.length} Seeds available`);
