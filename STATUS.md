...
| Competing capability realizations (R2) | equal checked faces admit distinct host implementations/artifacts; general hard requirements, explicit policy, stable realization characteristics, changing resource observations, finite compute ranges/service/topology, exact authority, and immutable selected Plan facts remain separate | three explicitly synthetic text-generation fixtures prove hard context/privacy rejection, deterministic policy choice, bounded candidate-decision evidence, minimum-first compute allocation, observation-driven replacement planning without old-Plan mutation, and non-AI video/storage representability | one selected deterministic fixture executes through ordinary planning, lowering, the fixed kernel, admitted host operation, exact provider, and presentation boundaries; normal std preparation validates scalable compute reservations | no new browser adapter or browser-owned realization truth; the required browser job only guards the existing adapter boundary | no firmware execution or bare-metal lane-provider claim; `BareMetal` is an architecture-neutral contract demonstrated below physical proof | remote-provider and network-storage cases are contract/fixture representations only; no live model API, endpoint, credential, paid service, or new transport claim | no physical CPU enumeration, scheduling-quality, firmware-lane, model, transport, or HIL proof |
...
- competing equal-face realization selection with exact implementation/artifact
  ...
  evidence, selected Plan sealing, ordinary deterministic-provider kernel
...
- architecture-neutral compute pools with minimum/preferred/maximum lanes,
  ...
  minimum-first allocation, hosted/bare-metal provider identity, scalable
  preparation validation, and no physical/provider lane identity in the Plan;
...
- canonical Programs 1–4 through checking, recursive expansion where required,
  ...
  host effects, terminal results, bounded evidence, and fail-closed negative
