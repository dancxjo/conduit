...
| Lossless form and composite boundary | S3 checkpoints: exact source, bounded lossless CST tokens, UTF-8 byte spans plus line/column locations, stable diagnostics, and an exact checked export boundary shared by composite offers and parent kinds | round-trip, spelling/semantic identity separation, recoverable invalid-line/EOF, source/token ceilings, missing/duplicate export denial, derived-offer equality, and ordinary parent-cord planning | parser/checker, planner, and hosted composite fixture; inline nesting remains open | no | no | fixture in-memory links only | no |
