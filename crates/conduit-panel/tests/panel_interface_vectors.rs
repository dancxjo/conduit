...
const FIXTURES: &str = include_str!("../../../conformance/c3/panel-interface-syntax-v1.json");
...
#[test]
fn interface_fixtures_cover_all_new_productions() {
    let suite: Value = serde_json::from_str(FIXTURES).unwrap();
    let covered: BTreeSet<&str> = suite["cases"]
        ...
        .collect();
    let required = BTreeSet::from(["interface", "implements"]);
    assert!(required.is_subset(&covered));
}

#[test]
fn interface_positive_production_seeds_have_lossless_malformed_recovery() {
    let suite: Value = serde_json::from_str(FIXTURES).unwrap();
