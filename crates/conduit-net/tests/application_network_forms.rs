...
use conduit_net::{
    ...
    DNS_RESOLVER_RESOURCE, DNS_RESOLVE_AUTHORITY, DNS_RESOLVE_KIND, NETWORK_CONNECTION_RESOURCE,
...
fn offers_keep_dns_and_connect_authority_distinct() {
    ...
    let dns = offers
        .iter()
        .find(|offer| offer.kind_id.as_str() == DNS_RESOLVE_KIND)
        .unwrap();
    let connect = offers
        .iter()
        .find(|offer| offer.kind_id.as_str() == NETWORK_CONNECT_KIND)
        .unwrap();
    assert_eq!(
        dns.resource_requirements[0].class_id.as_str(),
        DNS_RESOLVER_RESOURCE
    );
    assert_eq!(
        dns.authority_requirements[0].contract_id.as_str(),
        DNS_RESOLVE_AUTHORITY
    );
    assert_eq!(
        connect.resource_requirements[0].class_id.as_str(),
        NETWORK_CONNECTION_RESOURCE
    );
    assert_eq!(
        connect.authority_requirements[0].contract_id.as_str(),
        NETWORK_CONNECT_AUTHORITY
    );
    assert_ne!(
        dns.authority_requirements[0].contract_id,
        connect.authority_requirements[0].contract_id
    );
}
