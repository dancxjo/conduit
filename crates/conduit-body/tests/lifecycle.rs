...
fn body_survives_lull_and_one_wake_survives_replan() {
    ...
    let (awake, wake) = body.wake(9, SignId::from("woke")).unwrap();
    let plan_a = plan("plan-a");
    let wake = wake.plan_ready(&plan_a, SignId::from("planned-a")).unwrap();
    let play_a = bind_active_play(&plan_a.plan_id, &"host-a".into(), &"boot-a".into(), 1);
    let wake = wake
        .play_started(&play_a, SignId::from("playing-a"))
        .unwrap();
    let wake = wake
        .became_unsatisfied(&plan_a.plan_id, SignId::from("unsatisfied-a"))
        .unwrap();
    let plan_b = plan("plan-b");
    let wake = wake.plan_ready(&plan_b, SignId::from("planned-b")).unwrap();
    let play_b = bind_active_play(&plan_b.plan_id, &"host-b".into(), &"boot-b".into(), 2);
    let wake = wake
        .play_started(&play_b, SignId::from("playing-b"))
        .unwrap();
    assert_eq!(wake.plans[0].state, WakePlanState::Superseded);
    assert_ne!(wake.plans[0].active_play_id, wake.plans[1].active_play_id);
    let lulled_wake = wake.lull(SignId::from("lulled")).unwrap();
    let retained = awake
        .retain_after_lull(&lulled_wake, SignId::from("retained"))
        .unwrap();
    assert_eq!(retained.body_id, body_id);
    assert_eq!(retained.state, BodyState::Lulled);
    let (_, next_wake) = retained.wake(10, SignId::from("rewoke")).unwrap();
    assert_ne!(next_wake.wake_id, lulled_wake.wake_id);
}

#[test]
fn stale_inputs_and_duplicate_sign_fail_closed() {
    let (awake, wake) = body().wake(1, SignId::from("woke")).unwrap();
    assert_eq!(
    ...
    assert_eq!(
        awake.retain_after_lull(&wake, SignId::from("too-soon")),
...
#[test]
fn typed_events_are_the_exact_sign_history_and_tampering_fails_closed() {
    let (awake, wake) = body().wake(1, SignId::from("woke")).unwrap();
    assert!(matches!(
        awake.events.last(),
        Some(BodyLifecycleEvent::Woke { wake_id, sign_id })
            if wake_id == &wake.wake_id && sign_id.as_str() == "woke"
    ));
    let exact = plan("exact");
    let planned = wake.plan_ready(&exact, SignId::from("planned")).unwrap();
    assert!(matches!(
        planned.events.last(),
        Some(WakeLifecycleEvent::PlanReady { plan_id, sign_id })
            if plan_id == &exact.plan_id && sign_id.as_str() == "planned"
    ));
...
#[test]
fn wake_sign_history_is_finite() {
    let (_, wake) = body().wake(1, SignId::from("woke")).unwrap();
    let exact = plan("bounded");
    let waiting = wake.plan_ready(&exact, SignId::from("planned")).unwrap();
    let play = bind_active_play(&exact.plan_id, &"host".into(), &"boot".into(), 1);
    let mut playing = waiting
        .play_started(&play, SignId::from("playing"))
        .unwrap();
    for index in 3..MAX_WAKE_SIGNS {
        playing = playing
            .same_plan_observed(&exact.plan_id, SignId::from(format!("observation-{index}")))
            .unwrap();
    }
    assert_eq!(
        playing.same_plan_observed(&exact.plan_id, SignId::from("overflow")),
        Err(BodyLifecycleError::SignCapacityExhausted)
    );
}
