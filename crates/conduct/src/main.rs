...
fn run(
    ...
    let registry = if arguments.compatibility_demo {
        ...
        conduit_media::register_deterministic_signal_providers(&mut registry).map_err(|error| {
            cli_error(
                simple_diagnostic(error.code, &error.message),
                presentation,
                vec![source_document.clone()],
        ...
        conduit_spatial::register_deterministic_spatial_provider(&mut registry).map_err(
            |error| {
                cli_error(
                    simple_diagnostic(error.code, &error.message),
                    presentation,
        ...
        conduit_net::register_deterministic_network_providers(&mut registry).map_err(|error| {
            cli_error(
                simple_diagnostic(error.code, &error.message),
                presentation,
                vec![source_document.clone()],
