...
use conduit_compile::{
    ...
    COMPILE_INPUT_SCHEMA_VERSION, CandidateDocument, CompileInput, HostReportDocument,
    ImplementationDocument, PinDocument,
};
use conduit_core::{
    ARTIFACT_MANIFEST_SCHEMA_VERSION, ArtifactDigest, CAPABILITY_REPORT_SCHEMA_VERSION,
    EXECUTION_PLAN_SCHEMA_VERSION_V2, IMPLEMENTATION_MANIFEST_SCHEMA_VERSION, SemanticHash,
};
...
fn compile_emits_only_a_finite_validated_plan_result() {
    ...
    assert_eq!(result["result"]["schema"], "conduit.execution-plan/v2");
    assert_eq!(
...
fn compile_diagnostics_stay_on_stderr_and_reserved_path_is_disambiguated() {
    ...
    let disambiguated = command()
        ...
        .output()
        .unwrap();
    assert_eq!(disambiguated.status.code(), Some(2));
    assert!(String::from_utf8_lossy(&disambiguated.stderr).contains("cannot read compile"));
    std::fs::remove_dir_all(root).unwrap();
}
