...
fn reset_is_exact_authorized_non_actuating_and_advances_frame() {
    let host = HostId::from("std/netherwick");
    let boot = BootId::from("std/netherwick-boot");
    let offer_generation = OfferGeneration(7);
    let implementation_id = "netherwick/create1-observe-odometry@1";
    ...
    for (request, authority, expected) in [
        ...
        (
            ...
            Some(CreateOdometryResetAuthority {
                ...
                boot_id: &boot,
                offer_generation,
                implementation_id: "netherwick/other-odometry@1",
                valid_until_tick: 200,
            }),
