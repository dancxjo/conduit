...
pub const LAYOUT_CONTRACT_REVISION: &str = "conduit.std/layout-frame@1";
...
pub fn layout_contract_for(kind: &str) -> Option<StandardKindContract> {
    Some(match kind {
        LAYOUT_VIEWPORT_KIND => layout_viewport_contract(),
        LAYOUT_INSET_KIND => layout_inset_contract(),
        LAYOUT_ROW_KIND => layout_row_contract(),
        LAYOUT_COLUMN_KIND => layout_column_contract(),
        LAYOUT_STACK_KIND => layout_stack_contract(),
        LAYOUT_ALIGN_KIND => layout_align_contract(),
        _ => return None,
    })
}
