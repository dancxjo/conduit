...
fn readiness_contract_names_the_platform_stop_line() {
    ...
    for required in [
        ...
        "hosts/browser-runtime",
        "actual bounded WebSocket",
        "browser-side form execution",
        "Physical Pico LED acceptance",
        "loopback std-to-browser WebSocket",
        "live UDP",
    ] {
        assert!(
...
fn repository_status_matrix_keeps_proof_classes_distinct() {
    ...
    for required in [
        ...
        "WASM compilation alone is not browser execution",
        "Thumb compilation is not",
        "Chromium jobs execute the browser runtime",
        "one live loopback WebSocket",
        "frame/datagram fixtures remain distinct",
        "no UDP or physical proof",
    ] {
        assert!(
