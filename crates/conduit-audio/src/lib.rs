...
pub fn render_observed_alsa_null_panel(
    ...
    Ok(format!(
        "panel 0\n\n# Exact observed hosted ALSA proof; no ambient default device is permitted.\nnode capture : conduit.media/audio/capture {{\n{capture_config}}}\nnode process : conduit.media/audio/gain {{\n    lifecycle = \"standing\"\n    numeric_profile = \"pcm-s16-q15-round-nearest-away-saturate-no-nan-no-denormal-bit-exact\"\n    curve = \"linear-q15-absolute-frame\"\n    start_gain_q15 = 16384\n    end_gain_q15 = 16384\n    ramp_start_frame = 0\n    ramp_end_frame = 0\n    discontinuity = \"absolute-timeline\"\n    maximum_automation_points = 2\n    maximum_retained_samples = 0\n    maximum_frames = 32\n}}\nnode playback : conduit.media/audio/playback {{\n{playback_config}}}\n\ncord capture.frame -> process.frame {{ capacity = 2 max_value_bytes = 152 max_queued_bytes = 304 low_watermark = 0 high_watermark = 2 pressure = block }}\ncord process.frame -> playback.frame {{ capacity = 2 max_value_bytes = 152 max_queued_bytes = 304 low_watermark = 0 high_watermark = 2 pressure = block }}\n"
    ))
}
