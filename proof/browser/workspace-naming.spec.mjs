...
test('Workspace Birth binds all naming systems and keeps source metadata editable', async ({ page }) => {
  ...
  await expect(birth.getByRole('checkbox', { name: 'Morse Network', exact: true })).not.toBeChecked();
  ...
  await expect(name).toHaveAttribute('aria-describedby', /\S+/u);
  await expect(birth.getByLabel('Selected Conduit Form source', { exact: true })).toHaveAttribute('aria-describedby', /\S+/u);
  ...
  await expect(name).toHaveValue(/(?:son|dóttir|bur)$/u);
  ...
  await expect(name).toHaveValue(/ (?:ben|bat) /u);
