...
test("workflow topology keeps fast development separate from stable promotion", () => {
  ...
  assert.equal(finalizer.match(/gh pr list --state all/g)?.length, 2);
  assert.equal(finalizer.match(/git fetch --no-tags origin "refs\/pull\/\$pr_number\/head"/g)?.length, 2);
  assert.equal(finalizer.match(/test "\$\(git rev-parse FETCH_HEAD\)" = "\$HEAD_SHA"/g)?.length, 3);
  assert.match(finalizer, /git fetch --no-tags origin "\$merge_sha"/);
  assert.equal(finalizer.match(/base_sha=\$\(gh api "repos\/\$GITHUB_REPOSITORY\/pulls\/\$pr_number" --jq \.base\.sha\)/g)?.length, 2);
  assert.match(finalizer, /gh pr merge "\$pr_url" --merge --match-head-commit "\$HEAD_SHA"/);
  assert.equal(finalizer.match(/git merge-base --is-ancestor "\$base_sha" "\$merge_sha"/g)?.length, 2);
  assert.match(finalizer, /gh pr merge "\$pr_url" --merge --match-head-commit "\$HEAD_SHA"/);
