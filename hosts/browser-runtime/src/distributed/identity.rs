...
pub(super) fn advertisement_and_plan(
    ...
    let mut advertisement = match kind {
        PlanKind::StdBrowser => {
            conduit_signal_conformance::distributed_browser_sink_advertisement()
        }
        PlanKind::Triple => conduit_signal_conformance::triple::browser_advertisement(),
    };
    ...
    let plan = match (kind, source_identity) {
        (PlanKind::Triple, _) => {
            conduit_signal_conformance::triple::exact_plan().map(|exact| exact.plan)
        }
        (PlanKind::StdBrowser, Some((source_host, source_boot))) => {
            let (sink_host, sink_boot) = sink_identity.unwrap_or_else(|| {
                (
                    HostId::from(conduit_signal_conformance::DISTRIBUTED_BROWSER_HOST_ID),
                    BootId::from(conduit_signal_conformance::DISTRIBUTED_BROWSER_BOOT_ID),
                )
            });
            conduit_signal_conformance::exact_distributed_signal_plan_for_endpoints(
                source_host,
                source_boot,
                sink_host,
                sink_boot,
        ...
        (PlanKind::StdBrowser, None) => {
            conduit_signal_conformance::exact_distributed_signal_plan().map(|exact| exact.plan)
        }
    }?;
