...
use conduit_std_host::hosted_job::{
    run_bounded_job, AdmittedExecutable, JobCancellation,
};
...
fn std_host_executes_without_a_shell_and_keeps_environment_exact() {
    ...
    let report = run_bounded_job(&request, &executable(&request, "/usr/bin/env"), &JobCancellation::default()).unwrap();
...
fn stdout_is_drained_but_retained_only_to_the_declared_bound() {
    ...
    let report = run_bounded_job(&request, &executable(&request, "/usr/bin/yes"), &JobCancellation::default()).unwrap();
    assert_eq!(report.stdout.bytes.len(), 37);
    assert!(report.usage.stdout_observed_bytes >= 37);
    assert!(matches!(report.stdout.pressure, JobStreamPressure::Truncated { .. }));
    assert!(matches!(
        report.lifecycle.last(),
...
fn failure_cancellation_timeout_and_provider_loss_are_distinct() {
    ...
    let report = run_bounded_job(&failed, &executable(&failed, "/usr/bin/false"), &JobCancellation::default()).unwrap();
    assert!(matches!(
        report.lifecycle.last(),
        Some(JobLifecycleEvent::Terminal(JobTerminalOutcome::Failed { .. }))
    ));

    let cancelled = request("/usr/bin/printf", vec!["ignored".to_string()], vec![], 16, 1_000);
    let cancellation = JobCancellation::default();
    cancellation.cancel();
    let report = run_bounded_job(&cancelled, &executable(&cancelled, "/usr/bin/printf"), &cancellation).unwrap();
    assert!(matches!(
        report.lifecycle.last(),
    ...
    let report = run_bounded_job(&timeout, &executable(&timeout, "/usr/bin/sleep"), &JobCancellation::default()).unwrap();
    assert!(matches!(
        report.lifecycle.last(),
        Some(JobLifecycleEvent::Terminal(JobTerminalOutcome::TimedOut { .. }))
    ));

    let lost = request("/usr/bin/printf", vec!["ignored".to_string()], vec![], 16, 1_000);
    ...
    assert!(matches!(
        report.lifecycle.last(),
        Some(JobLifecycleEvent::Terminal(JobTerminalOutcome::ProviderLost { .. }))
    ));
}
...
fn request(
    ...
    JobRequest {
        executable: BoundedResourceRef {
            ...
            extent: ResourceExtent { bytes: 1, items: Some(1) },
