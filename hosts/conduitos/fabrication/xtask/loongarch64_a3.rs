...
const BINARY: &str = "conduitos-loongarch64-a3";
const KERNEL_PREFIX: &str = "CONDUIT_KERNEL_SIGN ";
const IDENTITY_PREFIX: &str = "CONDUIT_LOONGARCH64_A3_IDENTITY ";
...
pub fn run(opts: &GlobalOpts) -> Result<(), ConduitosError> {
    ...
    build_variant(opts, BINARY, "loongarch64-a3")?;
...
pub fn prove(opts: &GlobalOpts) -> Result<(), ConduitosError> {
    ...
    let paths = Paths::new(ConduitosArch::Loongarch64)?;
    build_variant(opts, BINARY, "loongarch64-a3")?;
    let first_image = image::assemble_architecture_proof(ConduitosArch::Loongarch64, opts)?;
    build_variant(opts, BINARY, "loongarch64-a3")?;
    let second_image = image::assemble_architecture_proof(ConduitosArch::Loongarch64, opts)?;
    ...
    if opts.json {
        ...
        println!("ConduitOS LoongArch64 A3 proof: {}", path.display());
    }
    Ok(())
}
...
pub(super) fn build_variant(
    ...
    let script = paths.root.join("hosts/conduitos/linker/loongarch64_a3.ld");
    ...
    command
        ...
        .env(
            "CONDUITOS_BUILD_ID",
            format!("conduitos-build/{commit}/loongarch64/v1"),
        )
    ...
    let status = command
        .status()
        .map_err(|e| refusal("loongarch64-a3-toolchain-unavailable", e.to_string()))?;
    if !status.success() {
        return Err(refusal(
            "loongarch64-a3-compile-link-failed",
            status.to_string(),
        ));
    }
...
pub(super) fn boot_transcript(paths: &Paths) -> Result<String, ConduitosError> {
    loongarch64_a1::boot_until(paths, IDENTITY_PREFIX)
}
...
pub(super) fn parse_one<T: for<'a> Deserialize<'a>>(
    ...
    if values.len() != 1 {
        return Err(refusal(
            "absent-or-duplicate-loongarch64-a3-sign",
            format!("expected one {name} Sign, found {}", values.len()),
        ));
    }
    serde_json::from_str(values[0].trim_end_matches('\r'))
        .map_err(|e| refusal("malformed-loongarch64-a3-sign", e.to_string()))
}
...
pub(super) fn validate(
    ...
    if kernel.schema != "conduit.conduitos.kernel-sign/v2"
        ...
        || kernel.arch != "loongarch64"
        || kernel.build_id != format!("conduitos-build/{commit}/loongarch64/v1")
        ...
        || identity.image_id != format!("conduitos-image/{commit}/loongarch64/v1")
...
fn reject_dry_run(opts: &GlobalOpts) -> Result<(), ConduitosError> {
    if opts.dry_run {
        Err(refusal(
            ...
            "LoongArch64 A3 requires emulator execution",
        ))
    } else {
        Ok(())
    }
