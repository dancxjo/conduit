...
test("HTML Patchbay reconstructs one typed state accessibly and survives delivery loss", async ({page,browser},testInfo) => {
  ...
  try {
    ...
    const semanticSubjects=snapshot.presentation.subjects.filter(item=>item.role==="Gear"||item.role==="Port"||item.role==="Diagnostic"||(item.role==="Cord"&&snapshot.presentation.properties.some(property=>property.subject===item.identity&&property.name==="source-port"))).map(item=>item.identity).sort();
