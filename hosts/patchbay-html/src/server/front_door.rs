...
impl PatchbayHtmlServer {
    pub fn bind_front_door(address: SocketAddr) -> Result<Self, ServerError> {
        let session = LocalFrontDoor::fresh().map_err(ServerError::Interaction)?;
        let snapshot = snapshot_for_front_door(&session).map_err(ServerError::Interaction)?;
        let mut server = Self::bind(address, &snapshot)?;
        server.front_door = Some(Arc::new(Mutex::new(session)));
        Ok(server)
    }
    ...
    pub(super) fn refresh_front_door(&mut self) -> Result<(), ServerError> {
        ...
        snapshot.interaction.selected_subject = Some(snapshot.entrance.selected_subject.clone());
