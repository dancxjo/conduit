...
export class BodyWebRtcSession {
  ...
  #requireTraffic(negotiationId, role) {
    if (negotiationId !== this.#grant.negotiation_id) throw new Error("stale negotiation identity");
    if (this.#grant.role !== role) throw new Error(`WebRTC ${role} role required`);
    if (!this.#sessionReady || this.#terminal !== null) throw new Error("WebRTC session is not current and Ready");
  }

  async #sendFrame(frame) {
    await this.#line.writable(frame.byteLength);
    const sent = this.#line.send(frame);
    if (!sent.accepted) throw new Error(`session frame send refused: ${sent.reason}`);
  }

  async offerValue(negotiationId, bytes) {
    this.#requireTraffic(negotiationId, "source");
    if (!(bytes instanceof Uint8Array) || bytes.byteLength === 0) throw new Error("invalid Cord value");
    if (this.#offered !== null || (this.#acceptedSequence !== null &&
        this.#acceptedSequence !== this.#deliveredSequence)) {
      throw new Error("Cord value already in flight");
    }
    const sequence = webRtcSessionNextSequence(this.#runtime);
    const status = offerWebRtcSessionValue(this.#runtime, bytes);
    if (status < 0) throw new Error(`Cord offer refused ${status}`);
    const frame = takeWebRtcSessionOutput(this.#runtime);
    if (frame === null) throw new Error("Cord offer emitted no frame");
    const result = new Promise((resolve, reject) => { this.#offered = { sequence, resolve, reject }; });
    try {
      await this.#sendFrame(frame);
    } catch (error) {
      this.#fail("traffic-send-refused", error);
      throw error;
    }
    return result;
  }
  ...
  receiveValue(negotiationId) {
    this.#requireTraffic(negotiationId, "sink");
    if (this.#receive !== null) throw new Error("Cord receive already pending");
    if (this.#received !== null) {
      if (this.#receivedExposed) throw new Error("Cord value already received");
      this.#receivedExposed = true;
      return Promise.resolve(this.#receivedResult());
    }
    return new Promise((resolve, reject) => { this.#receive = { resolve, reject }; });
  }

  pressureNextValue(negotiationId) {
    this.#requireTraffic(negotiationId, "sink");
    if (this.#pressureNext || this.#received !== null || this.#receive !== null) {
      throw new Error("Cord receive admission already pending");
    }
    this.#pressureNext = true;
  }

  async deliverValue(negotiationId, sequence) {
    this.#requireTraffic(negotiationId, "sink");
    if (this.#received === null || sequence !== this.#received.sequence) {
      throw new Error("Cord delivery identity refused");
    }
    const status = deliverWebRtcSessionValue(this.#runtime);
    if (status < 0) throw new Error(`Cord delivery refused ${status}`);
    const frame = takeWebRtcSessionOutput(this.#runtime);
    if (frame === null) throw new Error("Cord delivery emitted no frame");
    await this.#sendFrame(frame);
    this.#deliveredSequence = sequence;
    this.#received = null;
    this.#receivedExposed = false;
    return Object.freeze({ delivered: true, sequence });
  }

  waitDelivered(negotiationId, sequence) {
    this.#requireTraffic(negotiationId, "source");
    if (this.#deliveredSequence === sequence) return Promise.resolve(Object.freeze({ delivered: true, sequence }));
    if (this.#acceptedSequence !== sequence) throw new Error("Cord delivery sequence was not accepted");
    if (this.#deliveryWaiters.has(sequence)) throw new Error("Cord delivery wait already pending");
    return new Promise((resolve, reject) => this.#deliveryWaiters.set(sequence, { resolve, reject }));
  }

  #receivedResult() {
    return Object.freeze({ sequence: this.#received.sequence, bytes: Object.freeze([...this.#received.bytes]) });
  }

  async #pump() {
    try {
      while (this.#sessionReady && this.#terminal === null) {
        const received = await this.#line.receive();
        if (!received.ok) {
          this.#fail(`line-${received.reason}`);
          return;
        }
        if (this.#grant.role === "sink" && this.#pressureNext) {
          const status = pressureWebRtcSessionOffer(this.#runtime, received.bytes);
          if (status < 0) throw new Error(`Cord pressure refused ${status}`);
          this.#pressureNext = false;
          const pressure = takeWebRtcSessionOutput(this.#runtime);
          if (pressure === null) throw new Error("Cord pressure emitted no frame");
          await this.#sendFrame(pressure);
          continue;
        }
        const status = ingestWebRtcSession(this.#runtime, received.bytes);
        if (status < 0) throw new Error(`session traffic refused ${status}`);
        const event = webRtcSessionLastEvent(this.#runtime);
        const output = takeWebRtcSessionOutput(this.#runtime);
        if (output !== null) await this.#sendFrame(output);
        if (event === "offered") {
          const sequence = webRtcSessionNextSequence(this.#runtime);
          this.#acceptedSequence = sequence;
          this.#received = Object.freeze({ sequence, bytes: webRtcSessionValue(this.#runtime) });
          if (this.#receive !== null) {
            const waiter = this.#receive;
            this.#receive = null;
            this.#receivedExposed = true;
            waiter.resolve(this.#receivedResult());
          }
        } else if (event === "pressure" || event === "accepted") {
          if (this.#offered === null) throw new Error(`unexpected peer ${event}`);
          const offered = this.#offered;
          this.#offered = null;
          if (event === "accepted") this.#acceptedSequence = offered.sequence;
          offered.resolve(Object.freeze({
            accepted: event === "accepted",
            retryable: event === "pressure",
            reason: event === "pressure" ? "peer-pressure" : null,
            sequence: offered.sequence,
            delivered: false,
          }));
        } else if (event === "delivered") {
          const sequence = webRtcSessionNextSequence(this.#runtime) - 1;
          this.#deliveredSequence = sequence;
          const waiter = this.#deliveryWaiters.get(sequence);
          if (waiter !== undefined) {
            this.#deliveryWaiters.delete(sequence);
            waiter.resolve(Object.freeze({ delivered: true, sequence }));
          }
        }
      }
    } catch (error) {
      this.#fail("traffic-refused", error);
    }
  }
  ...
  state() {
    return Object.freeze({
      ...
      deliveredSequence: this.#deliveredSequence,
      valuePending: this.#received !== null,
      receivePending: this.#receive !== null,
      offerPending: this.#offered !== null,
    });
  }
  ...
  #fail(reason, cause) {
    ...
    this.#offered?.reject(cause ?? new Error(reason));
    this.#offered = null;
    this.#receive?.reject(cause ?? new Error(reason));
    this.#receive = null;
    for (const waiter of this.#deliveryWaiters.values()) waiter.reject(cause ?? new Error(reason));
