...
function render(snapshot){
  ...
  fillLines("#diagnostics ol",subjects("Diagnostic").flatMap(subject=>texts(subject.identity)));renderCards();fillLines("#topology ul",subjects("Host").flatMap(subject=>[subject.accessibility_name,...texts(subject.identity)]));fillLines("#linear ol",p.text.map(item=>item.text));displaySelection(snapshot.interaction.selected_subject??p.subjects[0]?.identity);
}
