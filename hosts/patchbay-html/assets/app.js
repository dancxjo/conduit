...
function displaySelection(identity){
  ...
  for(const item of properties(identity).filter(item=>item.name!=="semantic-id"&&!item.name.endsWith("-id"))){const name=item.name.startsWith("authored-control-")?"Authored configuration":item.name;term(summary,name,propertyText(item.value));}
  term(exactFacts,"Presentation subject",identity);for(const item of properties(identity).filter(item=>item.name==="semantic-id"||item.name.endsWith("-id")))term(exactFacts,item.name,propertyText(item.value));
...
function renderGraph(){
  ...
  let gears=subjects("Gear");if(state.reversed)gears=[...gears].reverse();const ports=subjects("Port"),cords=subjects("Cord").filter(item=>property(item.identity,"source-port")),diagnostics=subjects("Diagnostic");
  ...
  state.graphHeight=Math.max(360,235+Math.ceil(gears.length/3)*190);applyViewport();displaySelection(state.selected);
}
