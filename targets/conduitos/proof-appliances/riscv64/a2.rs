...
compile_error!("conduitos-riscv64-a2 must compile as RISC-V64");
...
#[unsafe(no_mangle)]
#[unsafe(link_section = ".text.conduitos_riscv64_a2_start")]
pub extern "C" fn conduitos_riscv64_a2_start() -> ! {
    ...
    if !arch::timer_arm() {
        refuse("unavailable-sbi-timer-mechanism");
    }
    stage("idle");
...
fn machine_sign(nonce: u64, lane: &AdmittedLane, idle_entries: u32) {
    let mut output = Output::new();
    output.push(b"CONDUIT_RISCV64_MACHINE_SIGN {\"schema\":\"conduit.conduitos.riscv64-a2-sign/v1\",\"status\":\"completed\",\"architecture\":\"riscv64\",\"boot_id\":\"boot-riscv64-");
    output.hex(nonce ^ 0x5256_3634_0000_0002);
    output.push(b"\",\"kernel\":\"conduit-kernel\",\"lane_id\":\"");
    output.push(LANE_ID.as_bytes());
    output.push(b"\",\"lane_count\":1,\"runtime_base_count\":4,\"runtime_memory_bytes\":4096,\"timer_slots\":1,\"interrupt_fact_slots\":1,\"wake_source\":\"riscv-supervisor-timer-interrupt\",\"wake_cause\":5,\"sbi_mechanism\":\"TIME/set_timer\",\"idle_entries\":");
...
fn stage(name: &str) {
    arch::present(b"CONDUIT_RISCV64_STAGE ");
...
fn refuse(reason: &str) -> ! {
    arch::disable_interrupts();
    arch::present(b"CONDUIT_RISCV64_REFUSAL ");
