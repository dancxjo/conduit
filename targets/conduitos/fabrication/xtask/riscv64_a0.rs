...
const ENTRY: &str = "conduitos_riscv64_a0_start";
...
pub fn execute(opts: &GlobalOpts) -> Result<BuildRecord, ConduitosError> {
    ...
    if opts.dry_run {
        println!("rustc --target {TARGET} object; rust-lld -m elf64lriscv");
    ...
    let object = paths.target.join("riscv64-entry.o");
    ...
    let inspection = Inspection {
        schema: "conduit.conduitos.riscv64-a0/v1",
    ...
    if !opts.quiet && !opts.json {
        println!("ConduitOS RISC-V64 A0 ELF: {}", paths.kernel.display());
    }
    Ok(record)
}
...
fn check_shared_backbone(paths: &Paths, opts: &GlobalOpts) -> Result<(), ConduitosError> {
    ...
    if status.success() {
        ...
        Err(refusal(
            ...
            "RISC-V64 shared backbone did not compile",
        ))
    }
}
...
fn compile_object(paths: &Paths, object: &Path) -> Result<(), ConduitosError> {
    let source = paths
        .root
        .join("targets/conduitos/proof/appliances/riscv64/a0.rs");
    let status = Command::new("rustc")
        .args([
            "--crate-name",
            "conduitos_riscv64_a0",
            "--crate-type",
            "bin",
            "--edition",
            "2024",
    ...
    if status.success() {
        ...
        Err(refusal("riscv64-object-compile-failed", status.to_string()))
    }
}
...
fn link(paths: &Paths, object: &Path) -> Result<(), ConduitosError> {
    ...
    let script = paths
        .root
        .join("targets/conduitos/proof/appliances/riscv64/linker/a0.ld");
    let status = Command::new(linker)
        ...
        .current_dir(&paths.root)
        .status()
        .map_err(|error| refusal("riscv64-linker-unavailable", error.to_string()))?;
    if status.success() {
        Ok(())
    } else {
        Err(refusal("riscv64-link-failed", status.to_string()))
    }
}
...
pub(super) fn rust_lld(root: &Path) -> Result<PathBuf, ConduitosError> {
    let sysroot = output(
        ...
        &["--print", "sysroot"],
        root,
        "riscv64-object-toolchain-unavailable",
    )?;
    let version = output(
        "rustc",
        &["-vV"],
    ...
    path.is_file()
        ...
        .ok_or_else(|| refusal("riscv64-linker-unavailable", path.display().to_string()))
}
...
fn inspect_readelf(paths: &Paths) -> Result<(), ConduitosError> {
    ...
    {
        return Err(invalid(
            "exact RISC-V64 entry absent or another architecture leaked into symbols",
        ));
    }
    let sections = output(
        "readelf",
...
fn inspect_elf(bytes: &[u8]) -> Result<ElfFacts, ConduitosError> {
    ...
    if u16_at(bytes, 16)? != 2 || u16_at(bytes, 18)? != EM_RISCV {
        return Err(invalid("artifact is not a RISC-V64 executable"));
    }
    let entry = u64_at(bytes, 24)?;
    let abi_flags = u32_at(bytes, 48)?;
    if entry == 0 || abi_flags & !0x7f != 0 || u16_at(bytes, 52)? != 64 || u16_at(bytes, 54)? < 56 {
        return Err(invalid("entry or RISC-V ELF ABI facts malformed"));
    }
    let phoff =
...
fn invalid(detail: impl Into<String>) -> ConduitosError {
    refusal("invalid-riscv64-a0-artifact", detail)
}
