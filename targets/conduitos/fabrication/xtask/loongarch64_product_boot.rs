//! Pinned two-boot acceptance for the LoongArch64 product Host.
...
const PRODUCT_PREFIX: &str = "CONDUIT_LOONGARCH64_PRODUCT ";
...
pub(super) fn boot_twice(
    ...
    if first["host_id"] == second["host_id"] || first["boot_id"] == second["boot_id"] {
        return Err(refusal(
            "stale-loongarch64-product-identity",
            "independent product boots reused HostId or BootId",
        ));
    }
    let snapshot_path = paths.target.join("loongarch64-product-observatory.json");
    fs::write(
        &snapshot_path,
...
fn boot_once(
    ...
    let text = loongarch64_a1::boot_until_image(paths, image, OBSERVATORY_PREFIX)?;
    let product = parse_one(&text, PRODUCT_PREFIX, "product")?;
    if product["schema"] != "conduit.conduitos/loongarch64-product@1"
        ...
        || product["presenter_implementation_id"] != "presenter/loongarch64-linear-uart@1"
    ...
    {
        return Err(refusal(
            "broken-loongarch64-product-observatory-correlation",
...
fn parse_one(text: &str, prefix: &str, name: &str) -> Result<serde_json::Value, ConduitosError> {
    ...
    if values.len() != 1 {
        return Err(refusal(
            "absent-or-duplicate-loongarch64-product-sign",
...
fn prove_patchbay(
    ...
    for required in [
        ...
        if required.is_empty() || !linear.contains(required) {
            return Err(refusal(
                "patchbay-loongarch64-product-projection-incomplete",
...
fn invalid(e: serde_json::Error) -> ConduitosError {
    refusal("loongarch64-product-proof-invalid", e.to_string())
}
