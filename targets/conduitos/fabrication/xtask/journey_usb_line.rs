...
pub(super) fn evidence(serial: &str) -> Result<Evidence, ConduitosError> {
    ...
    if records.len() != 1 {
        return Err(ConduitosError::refusal(
            ...
            "ordinary product must publish exactly one current FTDI Line",
        ));
    }
        ...
        || text(record, "sink_boot_id")? != EXPECTED_PEER_BOOT
    {
        return Err(ConduitosError::refusal(
            ...
            "FTDI Line does not name the connected bounded journey peer",
        ));
    }
    Ok(Evidence {
...
mod tests {
    ...
    fn evidence_requires_one_current_exact_peer() {
        let serial = format!(
            "CONDUIT_USB_LINE_SIGN {{\"status\":\"current\",\"line_id\":\"line/1\",\"binding_id\":\"binding/1\",\"base_instance_id\":\"base/1\",\"state_sign_id\":\"sign/1\",\"sink_host_id\":\"{EXPECTED_PEER_HOST}\",\"sink_boot_id\":\"{EXPECTED_PEER_BOOT}\"}}\n"
        );
