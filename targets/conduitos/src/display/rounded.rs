//! Fixed-radius rectangle rasterization, clipped without creating new borders.
...
pub(super) fn render(
    ...
    let tokens = &super::style::NATIVE_STYLE;
    let radius = i32::from(tokens.corner_radius);
    let border = i32::from(tokens.border_width);
    let width = i32::from(bounds.width);
    let height = i32::from(bounds.height);
    // The caller has intersected clip with both bounds and the target extent.
    // Each visible pixel is visited once, with no storage or work outside it.
