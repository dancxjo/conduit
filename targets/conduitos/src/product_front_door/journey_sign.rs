    ...
    let line = format!(
        ...
        json_identity(
            projection
                .source_document_id
                .as_ref()
                .map(conduit_core::SourceDocumentId::as_str)
        ),
        json_identity(
            projection
                .checked_form_id
                .as_ref()
                .map(conduit_core::CheckedFormId::as_str)
        ),
        json_identity(
            projection
                .expanded_form_id
                .as_ref()
                .map(conduit_core::ExpandedFormId::as_str)
        ),
        json_identity(
            projection
                .body_id
        ...
        json_identity(projection.loss_kind.map(|kind| kind.as_str())),
        json_identity(
            projection
                .loss_sign_id
                .as_ref()
        ...
        json_identity(projection.loss_kind.map(|kind| kind.recovery())),
        json_identity(
            projection
                .result_sign_id
                .as_ref()
        ...
        projection.result_omitted_bytes,
        projection.input_count,
        projection.kernel_sign_gap.map_or_else(
            || "null".into(),
            |gap| format!(
                "{{\"first_sequence\":{},\"last_sequence\":{},\"entries\":{}}}",
                gap.first_sequence, gap.last_sequence, gap.entries
            )
        ),
