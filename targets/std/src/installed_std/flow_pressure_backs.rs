...
pub(super) static FLOW_BACKPRESSURE_FACTORY: BackFactory = BackFactory {
...
pub(super) static FLOW_COALESCE_LATEST_FACTORY: BackFactory = BackFactory {
...
pub(super) struct FlowPressureBack;

impl<const PORTS: usize> StepBack<PORTS> for FlowPressureBack {
...
impl FlowPressureBack {}

fn backpressure_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
    validate_backpressure(placement)?;
    Ok(budget(placement))
}

fn coalesce_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
    validate_coalesce(placement)?;
    Ok(budget(placement))
}

fn budget(placement: &PlannedGear) -> BackBudget {
    BackBudget {
        value_items: 0,
        value_bytes: 0,
        host_requests: 0,
...
fn prepare_backpressure(
    ...
    validate_backpressure(placement)?;
    Ok(InstalledBack::FlowBackpressure(FlowPressureBack))
}
...
fn prepare_coalesce(
    ...
    validate_coalesce(placement)?;
    Ok(InstalledBack::FlowCoalesceLatest(FlowPressureBack))
}
...
mod tests {
    ...
    fn flow_pressure_operation_passes_values_and_closure() {
        let mut operation = FlowPressureBack;
        let value = ValueRef {
            slot: 1,
            generation: 1,
