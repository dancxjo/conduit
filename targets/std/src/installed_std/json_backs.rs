            ...
            summary_fields: fragment
                ...
                .map(|placement| {
                    if placement.kind_id.as_str() == conduit_web::JSON_BOOLEAN_SUMMARY_KIND {
                        super::json_summary_back::field_configuration(placement)
                            .ok()
                            .map(str::to_owned)
                    } else {
                        None
...
fn encode_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
    budget(placement, conduit_std_offers::json_encode_std_offer())
}
fn decode_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_encode(
    ...
    Ok(InstalledBack::Json(JsonBack {
        pending: None,
        next: 0,
    }))
}
...
pub(super) static JSON_COLLECTION_STEP_FACTORY: BackFactory = BackFactory {
...
fn collection_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
    budget(
        placement,
