...
impl<const PORTS: usize> StepBack<PORTS> for MathScalarBack {
...
pub(super) struct MathScalarBack {
...
fn clamp_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
    validate(placement, &conduit_std_offers::math_clamp_offer(), 2)?;
    transform_for(placement)?;
    Ok(budget())
}

fn scale_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
    validate(placement, &conduit_std_offers::math_scale_offer(), 1)?;
    transform_for(placement)?;
    Ok(budget())
}

fn deadband_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
    validate(placement, &conduit_std_offers::math_deadband_offer(), 1)?;
    transform_for(placement)?;
    Ok(budget())
}

fn budget() -> BackBudget {
    BackBudget {
        value_items: 1,
...
fn operation() -> InstalledBack {
    InstalledBack::MathScalar(MathScalarBack {
        pending: None,
        completed: false,
...
fn quantity_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_quantity(
    ...
    Ok(InstalledBack::MathScalar(MathScalarBack {
        pending: None,
        completed: false,
        input_bytes: SCALAR_ENCODED_LEN as u32,
...
fn quantity_info_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
