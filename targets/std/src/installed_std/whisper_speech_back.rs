...
pub(super) static CLIP_FACTORY: BackFactory = BackFactory {
    implementation_id: conduit_std_offers::WHISPER_CLIP_SPEECH_IMPLEMENTATION,
    budget: clip_budget,
    prepare: prepare_clip,
};

pub(super) struct WhisperSpeechBack {
    pending: bool,
    emitted: bool,
    maximum_input_bytes: u32,
}

impl<const PORTS: usize> StepBack<PORTS> for WhisperSpeechBack {
...
fn clip_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare(
    ...
    validate(placement)?;
    Ok(InstalledBack::WhisperSpeech(WhisperSpeechBack {
        pending: false,
        emitted: false,
