...
pub(super) static TICK_FACTORY: BackFactory = BackFactory {
    implementation_id: TICK_IMPLEMENTATION,
    budget: tick_budget,
    prepare: prepare_tick,
};

pub(super) static EVERY_FACTORY: BackFactory = BackFactory {
    implementation_id: TIME_EVERY_IMPLEMENTATION,
    budget: every_budget,
    prepare: prepare_every,
};

pub(super) struct TickBack {
    values: Vec<ValueRef>,
    waits: Vec<ValueRef>,
    next: usize,
...
impl<const PORTS: usize> StepBack<PORTS> for TickBack {
...
fn tick_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn every_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn tick_budget_for(configuration: &TickConfiguration) -> Result<BackBudget, String> {
...
fn prepare_every(
    ...
    Ok(InstalledBack::Tick(TickBack {
        values: Vec::new(),
        waits: vec![wait],
        next: 0,
        pending: None,
...
#[cfg(test)]
pub(super) static TEST_OBSERVER_FACTORY: BackFactory = BackFactory {
...
#[cfg(test)]
pub(super) struct TestObserverBack {
    pending: Option<RequestId>,
    next: u32,
}

#[cfg(test)]
impl<const PORTS: usize> StepBack<PORTS> for TestObserverBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some((request, outcome)) = io.host_completion() {
            if self.pending != Some(request)
                || outcome.disposition != HostCallDisposition::Completed
...
fn prepare_test_observer(
    ...
    Ok(InstalledBack::TestObserver(TestObserverBack {
        pending: None,
        next: 0,
    }))
}
