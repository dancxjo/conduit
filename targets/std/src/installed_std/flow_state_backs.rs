...
pub(super) static STATE_LATEST_SCALAR_FACTORY: BackFactory = BackFactory {
...
pub(super) static FLOW_TEE_SCALAR_FACTORY: BackFactory = BackFactory {
    implementation_id: conduit_std_offers::FLOW_TEE_SCALAR_IMPLEMENTATION,
    budget: flow_tee_budget,
    prepare: prepare_flow_tee,
};

pub(super) struct StateLatestScalarBack {
    held: Option<ValueRef>,
    released: Option<ValueRef>,
    retain_resumed: bool,
}

pub(super) struct FlowTeeScalarBack {
    pending: Option<ValueRef>,
    phase: u8,
}

impl<const PORTS: usize> StepBack<PORTS> for StateLatestScalarBack {
...
impl<const PORTS: usize> StepBack<PORTS> for FlowTeeScalarBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some(value) = io.input(PortId(0)) {
            if value.byte_len != SCALAR_ENCODED_LEN as u32 {
...
impl StateLatestScalarBack {}

impl FlowTeeScalarBack {}

fn state_latest_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
    validate_state_latest(placement)?;
    Ok(budget())
}

fn flow_tee_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_state_latest(
    ...
    validate_state_latest(placement)?;
    Ok(InstalledBack::StateLatestScalar(StateLatestScalarBack {
        held: None,
        released: None,
...
fn prepare_flow_tee(
    ...
    validate_flow_tee(placement)?;
    Ok(InstalledBack::FlowTeeScalar(FlowTeeScalarBack {
        pending: None,
        phase: 0,
    }))
}
...
mod tests {
    ...
    fn latest_replaces_one_retained_value_and_releases_on_close() {
        let mut operation = StateLatestScalarBack {
            held: None,
            released: None,
            retain_resumed: false,
        };
    ...
    fn tee_exposes_both_outputs_in_one_operation_transaction() {
        let mut operation = FlowTeeScalarBack {
            pending: None,
            phase: 0,
        };
        let mut io = StepIo::test_frame(
