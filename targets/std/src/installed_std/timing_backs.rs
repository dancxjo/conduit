...
pub(super) static TIME_DEBOUNCE_FACTORY: BackFactory = BackFactory {
    implementation_id: conduit_std_offers::TIME_DEBOUNCE_IMPLEMENTATION,
    budget: debounce_budget,
    prepare: prepare_debounce,
};

pub(super) static TIME_TIMEOUT_FACTORY: BackFactory = BackFactory {
...
impl<const PORTS: usize> StepBack<PORTS> for TimeoutBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some((request, outcome)) = io.host_completion() {
            if self.pending != Some(request)
                || outcome.output.is_some()
...
fn debounce_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn timeout_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_debounce(
    ...
    validate_debounce(placement)?;
    let configuration = timing_configuration::parse(placement, true)?;
    Ok(InstalledBack::TimeDebounce(DebounceBack {
...
fn prepare_timeout(
    ...
    Ok(InstalledBack::TimeTimeout(TimeoutBack {
        durations: store_durations(values, configuration)?,
        false_values,
        true_values,
        next_request: 0,
