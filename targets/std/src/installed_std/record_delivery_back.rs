...
pub(super) struct RecordDeliveryStatusBack {
    pending: bool,
    observations: u16,
}

impl<const PORTS: usize> StepBack<PORTS> for RecordDeliveryStatusBack {
...
fn prepare(
    ...
    validate(placement)?;
    Ok(InstalledBack::RecordDeliveryStatus(
        RecordDeliveryStatusBack {
            pending: false,
            observations: 0,
        },
    ))
