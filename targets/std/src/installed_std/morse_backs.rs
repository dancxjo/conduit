...
use super::text_backs::{TextPresentationBack, TextTransformBack};
...
pub(super) static TEXT_MORSE_FACTORY: BackFactory = BackFactory {
    implementation_id: conduit_std_offers::TEXT_MORSE_IMPLEMENTATION,
    budget: text_morse_budget,
    prepare: prepare_text_morse,
};

pub(super) static INDICATOR_PRESENTATION_FACTORY: BackFactory = BackFactory {
...
fn composition_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_composition(
    ...
    validate_composition(placement)?;
    Ok(InstalledBack::TextUpper(TextTransformBack::bounded_input(
        4,
...
fn text_morse_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_indicator(
    ...
    validate_indicator(placement)?;
    Ok(InstalledBack::TextPresentation(
        TextPresentationBack::bounded(4),
    ))
}
