...
pub(super) static SINGLETON: BackFactory = BackFactory {
    implementation_id: conduit_std_offers::RECORD_SINGLETON_STREAM_STD_IMPLEMENTATION,
    budget,
    prepare: prepare_singleton,
};
pub(super) static EXACTLY_ONE: BackFactory = BackFactory {
    implementation_id: conduit_std_offers::RECORD_EXACTLY_ONE_STD_IMPLEMENTATION,
    budget,
    prepare: prepare_exactly_one,
};

pub(super) struct RecordSingletonStreamBack {
    emitted: bool,
}

impl<const PORTS: usize> StepBack<PORTS> for RecordSingletonStreamBack {
...
impl RecordSingletonStreamBack {}

pub(super) struct RecordExactlyOneBack {
...
impl<const PORTS: usize> StepBack<PORTS> for RecordExactlyOneBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if self.emitted {
            return StepOutcome::Complete;
        }
...
fn prepare_singleton(
    ...
    validate(placement)?;
    Ok(InstalledBack::RecordSingletonStream(
        RecordSingletonStreamBack { emitted: false },
    ))
}
...
fn prepare_exactly_one(
    ...
    validate(placement)?;
    Ok(InstalledBack::RecordExactlyOne(RecordExactlyOneBack {
        held: None,
        released: None,
        emitted: false,
