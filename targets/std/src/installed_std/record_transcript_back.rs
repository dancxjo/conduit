...
pub(super) struct RecordTranscriptBack {
...
impl<const PORTS: usize> StepBack<PORTS> for RecordTranscriptBack {
    fn step(
        &mut self,
        io: &mut StepIo<PORTS>,
        ...
        for port in [PortId(0), PortId(1), PortId(2)] {
            ...
            let result = match index {
                0 | 1 => super::typed_record_back::typed_leaf(canonical, &self.framed_type)
                    .map_err(|_| ())
                    .and_then(|frame| {
                        self.transcript
                            .record(
                ...
                _ => super::typed_record_back::typed_leaf(canonical, &self.terminal_type)
                    .map_err(|_| ())
                    .and_then(|wire| {
...
fn prepare(
    ...
    Ok(InstalledBack::RecordTranscript(RecordTranscriptBack {
