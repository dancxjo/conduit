...
pub(super) struct SpeechWindowToClipBack {
...
impl<const PORTS: usize> StepBack<PORTS> for SpeechWindowToClipBack {
...
impl SpeechWindowToClipBack {}

pub(super) struct SpeechResultToEventStreamBack {
    pending: Option<RequestId>,
    emitted: bool,
}

impl<const PORTS: usize> StepBack<PORTS> for SpeechResultToEventStreamBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if self.emitted {
            return StepOutcome::Complete;
        }
...
impl SpeechResultToEventStreamBack {}
...
fn window_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn result_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_window(
    ...
    Ok(InstalledBack::SpeechWindowToClip(SpeechWindowToClipBack {
        pending: None,
        next_request: 0,
        trigger,
        closing: false,
...
fn prepare_result_stream(
    ...
    validate_result(placement)?;
    Ok(InstalledBack::SpeechResultToEventStream(
        SpeechResultToEventStreamBack {
            pending: None,
            emitted: false,
        },
    ))
