...
impl<const PORTS: usize> StepBack<PORTS> for FlowGateScalarBack {
...
fn flow_gate_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_flow_gate(
    ...
    validate_flow_gate(placement)?;
    Ok(InstalledBack::FlowGateScalar(FlowGateScalarBack {
        enabled: false,
        pending_enable: None,
        next_request: 0,
...
mod tests {
    ...
    fn gate_defaults_closed_then_tracks_exact_false_and_true_completions() {
        let mut gate = FlowGateScalarBack {
            enabled: false,
            pending_enable: None,
            next_request: 0,
    ...
    fn gate_cancellation_clears_pending_decode_and_update_bound_fails_closed() {
        ...
        let mut bounded = FlowGateScalarBack {
            enabled: false,
            pending_enable: None,
            next_request: 1,
            maximum_enable_updates: 1,
