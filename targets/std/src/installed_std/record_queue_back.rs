...
pub(super) struct RecordQueueBack {
    maximum_items: u64,
...
impl<const PORTS: usize> StepBack<PORTS> for RecordQueueBack {
    fn step(
        &mut self,
        io: &mut StepIo<PORTS>,
        ...
        if let Some(value) = io.input(PortId(0)) {
            ...
            let frame = match super::typed_record_back::typed_leaf(canonical, &self.framed_type) {
                Ok(frame) => frame,
...
fn prepare(
    ...
    Ok(InstalledBack::RecordQueue(RecordQueueBack {
        maximum_items,
        maximum_frame_bytes,
        accepted: 0,
