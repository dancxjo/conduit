...
impl<const PORTS: usize> StepBack<PORTS> for ThrottleBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some((request, outcome)) = io.host_completion() {
            if self.pending != Some(request)
                || outcome.output.is_some()
...
fn delay_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn throttle_budget(placement: &PlannedGear) -> Result<BackBudget, String> {
...
fn prepare_delay(
    ...
    Ok(InstalledBack::TimeDelay(DelayBack {
...
fn prepare_throttle(
    ...
    let maximum_values = configuration.maximum_values;
    Ok(InstalledBack::TimeThrottle(ThrottleBack {
        durations: store_durations(values, configuration)?,
        terminal_releases: Vec::with_capacity(maximum_values),
        next_request: 0,
        maximum_values,
