...
pub(super) struct BodyConversationContextBack {
    token: Option<ValueRef>,
    pending: bool,
    next_request: u32,
}

impl<const PORTS: usize> StepBack<PORTS> for BodyConversationContextBack {
...
impl BodyConversationContextBack {}
...
fn prepare(
    ...
    Ok(InstalledBack::BodyConversationContext(
        BodyConversationContextBack {
            token: Some(token),
            pending: false,
            next_request: 0,
        },
...
mod tests {
    ...
    fn current_source_requests_each_replacement_and_never_completes_after_first_value() {
        let mut operation = BodyConversationContextBack {
            token: Some(value(0)),
            pending: false,
            next_request: 0,
        };
