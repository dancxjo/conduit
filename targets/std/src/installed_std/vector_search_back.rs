...
pub(super) static EXACT_FACTORY: BackFactory = BackFactory {
    implementation_id: conduit_ai::EXACT_VECTOR_SEARCH_IMPLEMENTATION,
    budget,
    prepare,
};

pub(super) static HNSW_FACTORY: BackFactory = BackFactory {
    implementation_id: crate::hosted_vector_index::HOSTED_HNSW_IMPLEMENTATION_ID,
    budget,
    prepare,
};

pub(super) struct VectorSearchBack {
    maximum_input_bytes: u32,
    pending: bool,
    emitted: bool,
}

impl<const PORTS: usize> StepBack<PORTS> for VectorSearchBack {
...
fn prepare(
    ...
    validate(placement)?;
    Ok(InstalledBack::VectorSearch(VectorSearchBack {
