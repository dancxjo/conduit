...
impl BrowserBack {
    ...
    pub(crate) fn source(value: ValueRef) -> Self {
        Self::installed_step(SourceBack {
            value,
            emitted: false,
        })
    }
    ...
    pub(crate) fn host_source(
        ...
        Ok(Self::installed_step(HostSourceBack {
            request,
    ...
    pub(crate) fn application_state(
        ...
        Ok(Self::installed_step(ApplicationStateBack {
            initial,
            maximum_input_bytes,
            pending: None,
            next: 0,
    ...
    pub(crate) fn singleton_stream(maximum_bytes: u32) -> Self {
        Self::installed_step(SingletonStreamBack {
            maximum_bytes,
            emitted: false,
        })
    }
    ...
    pub(crate) fn exactly_one(maximum_bytes: u32) -> Self {
        Self::installed_step(ExactlyOneBack {
            maximum_bytes,
            held: None,
            emitted: false,
        })
    ...
    pub(crate) fn presentation(maximum_input_bytes: u32, _maximum_values: u32) -> Self {
        Self::installed_step(PresentationBack {
            maximum_input_bytes,
            next_request: 0,
            pending: None,
        })
    ...
    pub(crate) fn compare_scalar(
        ...
        Self::installed_step(CompareScalarBack {
            operator,
            operands: [None, None],
            closed: [false; 2],
    ...
    pub(crate) fn inactive() -> Self {
        Self::installed_step(InactiveBack)
    }

    pub(crate) fn select_scalar() -> Self {
        Self::installed_step(SelectScalarBack {
            selector: None,
...
impl<const PORTS: usize> StepBack<PORTS> for ApplicationStateBack {
...
impl<const PORTS: usize> StepBack<PORTS> for HostSourceBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some((request, outcome)) = io.host_completion() {
            if self.pending != Some(request)
                || outcome.disposition != HostCallDisposition::Completed
...
impl StepBack<{ super::BROWSER_PORTS_PER_GEAR }> for BrowserBack {
...
struct SingletonStreamBack {
    maximum_bytes: u32,
    emitted: bool,
}

impl<const PORTS: usize> StepBack<PORTS> for SingletonStreamBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some(value) = io.input(PortId(0)) {
...
struct ExactlyOneBack {
    maximum_bytes: u32,
    held: Option<ValueRef>,
    emitted: bool,
}

impl<const PORTS: usize> StepBack<PORTS> for ExactlyOneBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some(value) = io.input(PortId(0)) {
...
impl<const PORTS: usize> StepBack<PORTS> for UnaryBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some((request, outcome)) = io.host_completion() {
            if self.pending != Some(request) {
                return fail(2);
...
impl<const PORTS: usize> StepBack<PORTS> for PresentationBack {
    fn step(&mut self, io: &mut StepIo<PORTS>, _: &StepInputBytes<'_, PORTS>) -> StepOutcome {
        if let Some((request, outcome)) = io.host_completion() {
            if self.pending != Some(request) {
                return fail(3);
...
impl<const PORTS: usize> StepBack<PORTS> for SelectScalarBack {
    fn step(
        &mut self,
        io: &mut StepIo<PORTS>,
...
impl<const PORTS: usize> StepBack<PORTS> for CompareScalarBack {
    fn step(
        &mut self,
        io: &mut StepIo<PORTS>,
        input_bytes: &StepInputBytes<'_, PORTS>,
...
impl<const PORTS: usize> StepBack<PORTS> for InactiveBack {
