//! Typed Garden-state observation fixture, excluded from production installations.
...
pub(crate) const KIND: &str = "conduit-test/garden-state-sink";
...
pub(crate) fn offer() -> CapabilityOffer {
    CapabilityOffer {
        kind_id: KIND.into(),
        kind_contract_revision: "conduit-test/garden-state-sink@1".into(),
        capability_id: KIND.into(),
        startup_parameters: Vec::new(),
        shorthand: None,
        inputs: vec![PortDescriptor {
            port_id: port_id("state"),
            value_kind: conduit_semantic_catalog::garden_state_type()
                .profile()
                .expect("reviewed Garden state profile")
                .value_kind()
                .clone(),
...
fn present(_: &PlannedGear, input: &[u8]) -> Result<BrowserHostResult, String> {
    conduit_semantic_catalog::decode_garden_state(input)
        .map_err(|_| "Garden sink received malformed state".to_string())?;
