...
pub(in crate::form_runner) fn prepare_partition_scheduler(
        ...
        .flat_map(|(fragment, part)| part.nodes.iter().map(move |node| (*fragment, node)))
    {
        ...
        replay_controls[usize::from(node.node.0)] =
            ...
            .map(Box::new);
        template_stores[usize::from(node.node.0)] =
            crate::installed_browser::template_storage::PreparedTemplateStore::for_placement(
                placement,
            )?
            .map(Box::new);
        ...
        garden_steps[usize::from(node.node.0)] =
            crate::installed_browser::garden_step::PreparedGardenStep::for_placement(placement)?
                .map(Box::new);
        stroke_captures[usize::from(node.node.0)] =
            crate::installed_browser::stroke_capture::PreparedStrokeCapture::for_placement(
                placement,
            )?
            .map(Box::new);
