...
mod tests {
    ...
    fn compile_commands_are_locked_explicit_and_feature_derived() {
        let root = Path::new("/repo/targets/esp32/firmware/wroom-signal");
    ...
    fn formatting_check_is_manifest_exact_and_does_not_set_jobs() {
        let command = cargo_fmt(
            Path::new("/repo"),
            Path::new("/repo/targets/esp32/firmware/wroom-signal/Cargo.toml"),
            "format-firmware",
        );
        assert_eq!(
            command.args,
            [
                "fmt",
                "--manifest-path",
                "/repo/targets/esp32/firmware/wroom-signal/Cargo.toml",
                "--",
                "--check",
            ]
        );
