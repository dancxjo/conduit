...
pub(super) fn verify(line: &mut NativePathCdcLine, identity: &FirmwareIdentity) -> PicoResult<()> {
    let mut raw = [0_u8; 2048];
    line.send_raw_stream_frame(BOOTSEL_QUERY, Duration::from_secs(2))?;
    let challenge = line.receive_raw_stream_frame(&mut raw, Duration::from_secs(3))?;
