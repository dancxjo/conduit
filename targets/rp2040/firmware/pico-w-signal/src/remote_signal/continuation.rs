...
pub async fn resume_plan_c_signal_sink(
    ...
    let local_len =
        encode_session_checkpoint_into(state.machine.checkpoint_offer(), &mut local_bytes, 2048)?;
    link.send_raw_stream_frame(&local_bytes[..local_len])
        .await?;
    ...
    sign.write_terminal(true, state.identity.terminal(), runtime)
        .await?;
    Ok(())
}
