...
const TCP_BUFFER_BYTES: usize = conduit_r1_network_conformance::R1_MAXIMUM_FRAME_BYTES as usize + 256;
...
pub async fn run(
    ...
    let identity = WebSocketRouteIdentity {
        ...
        websocket_link,
        address,
        port: conduit_r1_network_conformance::R1_WEBSOCKET_PORT,
        sign_id: conduit_r1_network_conformance::R1_WEBSOCKET_ROUTE_SIGN_ID,
    };
    ...
    if link
        .send_raw_stream_frame(conduit_r1_network_conformance::R1_WEBSOCKET_BASE_READY)
        .await
        .is_err()
    {
        remain_bootsel(link).await
    ...
    match link.receive_raw_stream_frame(&mut frame).await {
        Ok(raw) if raw == conduit_r1_network_conformance::R1_WEBSOCKET_ENDPOINT_SIGN_READY => {}
    ...
    if socket.accept(conduit_r1_network_conformance::R1_WEBSOCKET_PORT).await.is_err() {
        remain_bootsel(link).await
    }
...
async fn await_query(link: &mut UsbLinkSession) -> Result<bool, ()> {
    ...
    loop {
        ...
        if raw == conduit_r1_network_conformance::R1_WEBSOCKET_BASE_QUERY {
            return Ok(false);
        }
        if raw == conduit_r1_network_conformance::R1_PLAN_C_WEBSOCKET_BASE_QUERY {
            return Ok(true);
        }
        return Err(());
    }
