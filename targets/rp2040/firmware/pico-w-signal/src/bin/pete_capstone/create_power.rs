...
use conduit_create_oi::{
    CreatePowerPulseProfile, CreatePowerPulseProgress, CreatePowerToggle, CreatePowerToggleProvider,
};
...
pub async fn serve(class: &mut InertCdc) {
    if STATE
        .compare_exchange(IDLE, PREPARED, Ordering::AcqRel, Ordering::Acquire)
        .is_err()
    {
        return;
    }
    ...
    if send_control_frame(class, accepted.as_bytes())
        .await
        .is_err()
    {
