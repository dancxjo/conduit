        ...
        .is_ok()
    {
        ...
        loop {
            match link.receive_raw_stream_frame(&mut frame).await {
                Ok(raw) if raw == conduit_r1_network_conformance::R1_USB_NETWORK_SESSION_QUERY => {
                    // End the host's CDC 0 readiness wait before writing the
                    // authoritative typed failure Sign on CDC 1.
                    if link
                        .send_raw_stream_frame(conduit_r1_network_conformance::R1_USB_NETWORK_SESSION_FAILED)
                        .await
                        .is_ok()
                    {
                        ...
                        if !matches!(
                            ready,
                            Ok(raw) if raw == conduit_r1_network_conformance::R1_USB_NETWORK_FAILURE_SIGN_READY
                        ) {
                            break;
                        }
                        ...
                        let status = match sign
                            ...
                            .await
                        {
                            Ok(()) => conduit_r1_network_conformance::R1_USB_NETWORK_FAILURE_SIGN_WRITTEN,
                            Err(UsbSignError::FormatOverflow) => {
                                conduit_r1_network_conformance::R1_USB_NETWORK_FAILURE_SIGN_FORMAT_FAILED
                            }
                            Err(UsbSignError::Disconnected) => {
                                conduit_r1_network_conformance::R1_USB_NETWORK_FAILURE_SIGN_DISCONNECTED
                            }
                        };
